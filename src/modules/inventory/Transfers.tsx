          ...
          <tbody>
            {t.lines.map((l, i) => { const item = db.find<Item>(C.items, l.itemId); const pos = l.itemId ? engine.stockPosition(l.itemId, t.fromWarehouseId, { batch: l.batch || undefined }) : undefined; return (
              <tr key={l.id} className={pos && l.qty > pos.available ? 'error-row' : ''}>
                ...
                <td className="right money" style={{ color: pos && l.qty > pos.available ? 'var(--danger)' : 'var(--ink-3)' }}>{pos ? `${fmtQty(pos.available)} (${fmtQty(pos.onHand)} on hand)` : '—'}</td>
                <td><NumberField size="grid" value={l.qty} onChange={(v) => upd(l.id, { qty: v })} decimals={3} min={0} /></td>
                <td style={{ fontSize: 12 }}>{l.uom}</td>
                <td>{item?.tracking === 'Batch' ? <select className="field-input grid" value={l.batch ?? ''} onChange={(e) => upd(l.id, { batch: e.target.value || undefined })}><option value="">Any batch</option>{A.batchBalances(item.id, t.fromWarehouseId).filter((b) => b.qty > 0).map((b) => <option key={b.batch} value={b.batch}>{b.batch} ({fmtQty(b.qty)})</option>)}</select> : item?.tracking === 'Serial' ? <input className="field-input grid" placeholder="Serials, comma-separated" value={(l.serials ?? []).join(',')} onChange={(e) => upd(l.id, { serials: e.target.value.split(',').map((x) => x.trim()).filter(Boolean) })} /> : <span style={{ color: 'var(--ink-5)', fontSize: 12 }}>—</span>}</td>
            ...
            {t.lines.length === 0 && <tr><td colSpan={9} style={{ textAlign: 'center', color: 'var(--ink-3)', height: 64 }}>No lines</td></tr>}
          </tbody>
        </table>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 16px', borderTop: '1px solid var(--line)', background: 'var(--surface-2)', fontSize: 12, color: 'var(--ink-3)' }}><button type="button" className="btn-link" onClick={() => set({ lines: [...t.lines, { ...engine.newLine({ warehouseId: t.fromWarehouseId }) } as TransferLine] })}>+ Add line</button><span>Transfer value <strong style={{ color: 'var(--ink)' }}>{fmtMoney(t.totalValue)}</strong> at source AVCO</span></div>
      </div>
    </div>
  );
}
