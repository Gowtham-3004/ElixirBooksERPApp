  ...
  return (
    <div className="page">
      ...
      <DataTable rows={rows} rowKey={(r) => r.key} dense columns={[
        ...
        { key: 'account', label: 'Inventory account', render: (r) => <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{db.find<Account>(C.accounts, r.item.inventoryAccountId ?? IDS.accInvFG)?.code}</span> },
        ...
        { key: 'std', label: 'Std / purchase', align: 'right', render: (r) => <span className="money" style={{ color: 'var(--ink-3)' }}>{fmtMoney(r.item.standardCost ?? r.item.purchasePrice)}</span> },
