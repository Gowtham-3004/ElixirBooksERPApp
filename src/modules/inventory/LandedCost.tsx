    ...
    <div className="page">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div className="card" style={{ padding: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 280, overflow: 'auto' }}>{grns.map((g) => <label key={g.id} style={{ display: 'flex', gap: 10, alignItems: 'center', fontSize: 13, padding: '6px 8px', border: `1px solid ${lc.grnIds.includes(g.id) ? '#325CFF' : '#EAEAEA'}`, borderRadius: 8 }}><input type="checkbox" className="checkbox" checked={lc.grnIds.includes(g.id)} onChange={() => toggleGrn(g.id)} /><TwoLine primary={`${g.number} · ${g.partyName}`} secondary={`${fmtDate(g.date)} · ${g.lines.length} line(s) · ${fmtMoney(g.totals.taxable)}${g.landedCostIds?.length ? ' · already has landed cost' : ''}`} /></label>)}{grns.length === 0 && <div style={{ fontSize: 12, color: '#5F6368' }}>No posted GRNs</div>}</div>
        </div>
      ...
      <div className="card" style={{ padding: 16 }}>
        ...
        <div style={{ marginTop: 8, fontSize: 12, color: '#5F6368' }}>Total to allocate <strong style={{ color: '#0A0A0A' }}>{fmtMoney(lc.totalCost)}</strong> · the charge was already expensed on its own invoice; posting moves it into inventory (Cr the expense account)</div>
      </div>
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ padding: '10px 16px', borderBottom: '1px solid #EAEAEA', fontWeight: 600, fontSize: 13 }}>Allocation preview · {lc.allocations.length} stock line(s)</div>
        {lc.allocations.length === 0 ? <div style={{ padding: 16, fontSize: 13, color: '#5F6368' }}>Select at least one GRN with accepted stock lines.</div> : <table className="data-table dense"><thead><tr><th>GRN</th><th>Item</th><th>Warehouse</th><th className="right">Qty</th><th className="right">Line value</th><th className="right">Weight</th><th className="right">Allocated</th><th className="right">Rate before</th><th className="right">Rate after</th></tr></thead><tbody>{lc.allocations.map((a) => <tr key={a.id}><td className="identifier">{a.grnNumber}</td><td>{a.itemName}</td><td>{whName(a.warehouseId)}</td><td className="right money">{fmtQty(a.qty)}</td><td className="right money">{fmtMoney(a.baseValue)}</td><td className="right money">{a.weight ? fmtQty(a.weight) : '—'}</td><td className="right money" style={{ fontWeight: 600, color: '#12784E' }}>{fmtMoney(a.allocated)}</td><td className="right money">{fmtMoney(a.qty ? a.baseValue / a.qty : 0)}</td><td className="right money" style={{ fontWeight: 600 }}>{fmtMoney(a.newRate)}</td></tr>)}</tbody><tfoot><tr><td colSpan={6}>Total</td><td className="right money">{fmtMoney(lc.allocations.reduce((x, a) => x + a.allocated, 0))}</td><td colSpan={2} /></tr></tfoot></table>}
      </div>
    </div>
  );
}
