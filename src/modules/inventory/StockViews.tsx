...
const TYPE_COLOR: Record<string, string> = { GRN: '#12784E', Delivery: '#C0393F', Adjustment: '#8A4B0F', 'Transfer Out': '#3E5BA5', 'Transfer In': '#3E5BA5', Opening: '#5F6368', 'Purchase Return': '#C0393F', 'Sales Return': '#12784E', 'Landed Cost': '#A855F7', Scrap: '#9B1B21', 'POS Sale': '#C0393F', 'POS Return': '#12784E' };
export function TypeChip({ type }: { type: string }) { return <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 6px', borderRadius: 4, background: `${TYPE_COLOR[type] ?? '#5F6368'}18`, color: TYPE_COLOR[type] ?? '#5F6368', whiteSpace: 'nowrap' }}>{type}</span>; }
...
export function StockOnHand({ itemId }: { itemId?: string }) {
  ...
  const columns: Column<Row>[] = [
    ...
    { key: 'group', label: 'Group', sortable: true, value: (r) => r.item.group, render: (r) => <span style={{ fontSize: 12, color: '#5F6368' }}>{r.item.group}</span> },
    ...(wh ? [] : [{ key: 'wh', label: 'Warehouses', render: (r: Row) => <span style={{ fontSize: 12, color: '#5F6368' }}>{whs.filter((w) => engine.stockPosition(r.item.id, w.id).onHand !== 0).map((w) => `${w.name} ${fmtQty(engine.stockPosition(r.item.id, w.id).onHand)}`).join(' · ') || '—'}</span> }]),
    { key: 'uom', label: 'UOM', render: (r) => <span style={{ fontSize: 12, color: '#5F6368' }}>{r.item.baseUom}</span> },
    ...
    { key: 'available', label: 'Available', align: 'right', sortable: true, render: (r) => <span className="money" style={{ fontWeight: 600, color: r.low ? '#C0393F' : '#12784E' }}>{fmtQty(r.available)}{r.low && <Pill tone="warning">Low</Pill>}</span> },
    { key: 'committed', label: 'On order', align: 'right', render: (r) => <span className="money" style={{ color: r.committed ? '#325CFF' : '#B0B5BF' }}>{r.committed ? fmtQty(r.committed) : '—'}</span> },
    { key: 'inTransit', label: 'In transit', align: 'right', render: (r) => <span className="money" style={{ color: r.inTransit ? '#325CFF' : '#B0B5BF' }}>{r.inTransit ? fmtQty(r.inTransit) : '—'}</span> },
    { key: 'projected', label: 'Projected', align: 'right', render: (r) => <span className="money">{fmtQty(r.projected)}</span> },
    { key: 'reorder', label: 'Reorder lvl', align: 'right', render: (r) => <span className="money" style={{ fontSize: 12, color: '#5F6368' }}>{fmtQty(r.item.reorderLevel)}</span> },
...
function ItemDrill({ itemId, onClose }: { itemId: string; onClose: () => void }) {
  ...
  return (
    <Drawer open onClose={onClose} title={item.name} subtitle={`${item.code} · ${item.group} · ${item.tracking === 'None' ? 'no tracking' : item.tracking + '-tracked'}`} width={820} headerRight={<Button size="sm" onClick={() => nav.go(`masters/items/${itemId}`)}>Item master</Button>}>
      ...
      {tab === 'moves' && <div className="card" style={{ marginTop: 12, overflow: 'auto', maxHeight: 460 }}><table className="data-table dense"><thead><tr><th>Date</th><th>Type</th><th>Reference</th><th>Warehouse</th><th>Batch</th><th className="right">In</th><th className="right">Out</th><th className="right">Rate</th><th className="right">Value</th></tr></thead><tbody>{moves.map((m) => <tr key={m.id} className="clickable" onClick={() => { const l = sourceLink(m); if (l) nav.go(l); }}><td>{fmtDate(m.date)}</td><td><TypeChip type={m.type} /></td><td className="identifier link">{m.sourceNumber}</td><td>{m.warehouseName}{m.bin ? ` · ${m.bin}` : ''}</td><td className="identifier">{m.batch ?? (m.serials?.length ? `${m.serials.length} sn` : '—')}</td><td className="right money" style={{ color: '#12784E' }}>{m.baseQty > 0 ? fmtQty(m.baseQty) : '—'}</td><td className="right money" style={{ color: '#C0393F' }}>{m.baseQty < 0 ? fmtQty(-m.baseQty) : '—'}</td><td className="right money">{fmtMoney(m.rate)}</td><td className="right money">{fmtMoney(m.value)}</td></tr>)}</tbody></table></div>}
      ...
      {tab === 'wh' && <div className="card" style={{ marginTop: 12, overflow: 'hidden' }}><table className="data-table dense"><thead><tr><th>Warehouse</th><th className="right">On hand</th><th className="right">Reserved</th><th className="right">Available</th><th className="right">Avg rate</th><th className="right">Value</th></tr></thead><tbody>{whs.map((w) => { const p = engine.stockPosition(itemId, w.id); const v = A.valuation(itemId, w.id); if (!p.onHand && !p.reserved) return null; return <tr key={w.id}><td>{w.name} <span style={{ fontSize: 11, color: '#5F6368' }}>{w.type !== 'Standard' ? w.type : ''}</span></td><td className="right money">{fmtQty(p.onHand)}</td><td className="right money">{fmtQty(p.reserved)}</td><td className="right money">{fmtQty(p.available)}</td><td className="right money">{fmtMoney(v.avgRate)}</td><td className="right money">{fmtMoney(v.value)}</td></tr>; })}</tbody></table></div>}
    </Drawer>
  );
}
...
export function StockLedger({ params }: { params: Record<string, string> }) {
  ...
  return (
    <RegisterPage<StockMovement & { running: number }> title="Stock ledger" subtitle={<ScopeLine extra={`${rows.length} movements · immutable · FR-INV-001`} />} entity="movements" rows={rows} searchKeys={['itemName', 'itemCode', 'sourceNumber', 'batch', 'warehouseName']} pageSize={50}
      ...
      columns={[
        { key: 'date', label: 'Date', sortable: true, render: (m) => <span style={{ fontSize: 12, color: '#5F6368' }}>{fmtDate(m.date)}</span> },
        ...
        { key: 'in', label: 'In', align: 'right', render: (m) => <span className="money" style={{ color: m.baseQty > 0 ? '#12784E' : '#B0B5BF' }}>{m.baseQty > 0 ? fmtQty(m.baseQty) : '—'}</span> },
        { key: 'out', label: 'Out', align: 'right', render: (m) => <span className="money" style={{ color: m.baseQty < 0 ? '#C0393F' : '#B0B5BF' }}>{m.baseQty < 0 ? fmtQty(-m.baseQty) : '—'}</span> },
        ...
        { key: 'sourceType', label: 'Source', render: (m) => <span style={{ fontSize: 12, color: '#5F6368' }}>{m.sourceType}{m.reversalOfId ? ' (reversal)' : ''}</span> },
      ]}
  ...
  return (
    <RegisterPage title="Batches & serials" subtitle={<ScopeLine extra={`${rows.length} balances · ${expiring.length} expiring within 90 days`} />} entity="batches" rows={rows} rowKey={(r) => r.key} searchKeys={['ref', 'item.name', 'item.code', 'warehouseName']}
      ...
      columns={[
        ...
        { key: 'expiryDate', label: 'Expiry', render: (r: any) => { if (!r.expiryDate) return <span style={{ color: '#B0B5BF' }}>—</span>; const d = daysBetween(today(), r.expiryDate); return <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center', fontSize: 12 }}>{fmtDate(r.expiryDate)}{d < 0 ? <Pill tone="critical">Expired</Pill> : d <= 90 ? <Pill tone="warning">{d} d</Pill> : <Pill tone="good">OK</Pill>}</span>; } },
    ...
    <>
      <RegisterPage<Reservation> title="Reservations" subtitle={<ScopeLine extra={`${mine.filter((r) => r.status === 'Reserved' || r.status === 'Partially Fulfilled').length} active · FR-INV-004`} />} entity="reservations" rows={mine} searchKeys={['sourceNumber']}
        ...
        columns={[
          ...
          { key: 'available', label: 'Available now', align: 'right', render: (r) => { const p = engine.stockPosition(r.itemId, r.warehouseId); return <span className="money" style={{ color: p.onHand < r.qty - r.fulfilledQty ? '#C0393F' : '#12784E' }}>{fmtQty(p.onHand)} on hand</span>; } },
          { key: 'expiresAt', label: 'Expires', render: (r) => r.expiresAt ? <span style={{ fontSize: 12, color: daysBetween(today(), r.expiresAt.slice(0, 10)) < 0 ? '#C0393F' : undefined }}>{fmtDateTime(r.expiresAt)}</span> : <span style={{ color: '#B0B5BF' }}>—</span> },
