  ...
  return (
    <RegisterPage<A.Suggestion> title="Replenishment" subtitle={<ScopeLine extra={`${sugs.length} suggestion(s) · projected = on hand − reserved + on order + in transit vs max(reorder level, safety stock)`} />} entity="suggestions" rows={sugs} rowKey={(r) => r.item.id} searchKeys={['item.name', 'item.code', 'supplierName']}
      ...
      columns={[
        ...
        { key: 'committed', label: 'On order', align: 'right', render: (r) => <span className="money" style={{ color: 'var(--accent)' }}>{fmtQty(r.committed + r.inTransit)}</span> },
        { key: 'projected', label: 'Projected', align: 'right', sortable: true, render: (r) => <span className="money" style={{ fontWeight: 600, color: 'var(--danger)' }}>{fmtQty(r.projected)}</span> },
        { key: 'reorderLevel', label: 'Reorder / safety', align: 'right', render: (r) => <span className="money" style={{ fontSize: 12, color: 'var(--ink-3)' }}>{fmtQty(r.reorderLevel)} / {fmtQty(r.safetyStock)}</span> },
