  ...
  return (
    <RegisterPage<StockAdjustment> title="Stock adjustments" subtitle={`${mine.length} adjustments · ${rule ? `${rule.name} (${rule.conditions.map((c: any) => `${c.field} ${c.op} ${c.value}`).join(', ')})` : 'no approval rule'}`} entity="adjustments" rows={mine} searchKeys={['number', 'reason', 'warehouseName']}
      columns={[
        ...
        { key: 'warehouseName', label: 'Warehouse', sortable: true, render: (r) => <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{r.warehouseName}</span> },
        { key: 'adjustmentType', label: 'Type', sortable: true, render: (r) => <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{r.adjustmentType}</span> },
        { key: 'qty', label: 'Qty change', align: 'right', render: (r) => { const q = r.lines.reduce((x, l) => x + l.qty, 0); return <span className="money" style={{ fontWeight: 600, color: q < 0 ? 'var(--danger)' : 'var(--good)' }}>{q > 0 ? '+' : ''}{fmtQty(q)}</span>; } },
        { key: 'reason', label: 'Reason', render: (r) => <span style={{ fontSize: 12, color: 'var(--ink-3)', maxWidth: 220, display: 'inline-block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.reason}</span> },
          ...
          <tbody>
            {a.lines.map((l, i) => { const item = db.find<Item>(C.items, l.itemId); const pos = l.itemId ? engine.stockPosition(l.itemId, a.warehouseId, { batch: l.batch || undefined }) : undefined; return (
              <tr key={l.id} className={l.qty < 0 && pos && pos.onHand + l.qty < 0 ? 'error-row' : ''}>
                ...
                <td className="right money" style={{ color: 'var(--ink-3)' }}>{pos ? fmtQty(pos.onHand) : '—'}</td>
                ...
                <td>{item?.tracking === 'Batch' ? <input className="field-input grid" placeholder="Batch" value={l.batch ?? ''} onChange={(e) => upd(l.id, { batch: e.target.value })} /> : item?.tracking === 'Serial' ? <input className="field-input grid" placeholder="Serials, comma-separated" value={(l.serials ?? []).join(',')} onChange={(e) => upd(l.id, { serials: e.target.value.split(',').map((x) => x.trim()).filter(Boolean) })} /> : <span style={{ color: 'var(--ink-5)', fontSize: 12 }}>—</span>}</td>
                ...
                <td className="right money" style={{ color: l.qty < 0 ? 'var(--danger)' : 'var(--good)' }}>{l.qty < 0 ? '−' : ''}{fmtMoney(l.value)}</td>
                <td><button type="button" className="btn-icon" onClick={() => set({ lines: a.lines.filter((x) => x.id !== l.id) })}>✕</button></td>
              </tr>); })}
            {a.lines.length === 0 && <tr><td colSpan={10} style={{ textAlign: 'center', color: 'var(--ink-3)', height: 64 }}>No lines — add an item</td></tr>}
          </tbody>
        </table>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 16px', borderTop: '1px solid var(--line)', background: 'var(--surface-2)', fontSize: 12, color: 'var(--ink-3)' }}><button type="button" className="btn-link" onClick={() => set({ lines: [...a.lines, { ...engine.newLine({ warehouseId: a.warehouseId }), value: 0 } as AdjustmentLine] })}>+ Add line</button><span>Adjustment value <strong style={{ color: 'var(--ink)' }}>{fmtMoney(a.totalValue)}</strong> · journal Dr/Cr Inventory vs 5020 Inventory adjustments</span></div>
      </div>
  ...
  return (
    <>
      <DocumentPage backLabel="Adjustments" onBack={() => nav.go('inventory/adjustments')} number={a.number} badges={<><Badge status={a.status === 'Submitted' ? 'Pending Approval' : a.status}>{a.status === 'Submitted' ? 'Pending approval' : a.status}</Badge><Badge status="Draft">{a.adjustmentType}</Badge></>} amount={{ label: 'Adjustment value', value: a.totalValue, currency: s.currency }}
        ...
        tabs={[
          { id: 'lines', label: 'Lines', content: <div className="card" style={{ overflow: 'hidden' }}><table className="data-table dense"><thead><tr><th>#</th><th>Item</th><th className="right">Qty</th><th>UOM</th><th>Batch / serial</th><th className="right">Rate</th><th className="right">Value</th><th>Reason</th></tr></thead><tbody>{a.lines.map((l, i) => <tr key={l.id}><td>{i + 1}</td><td><ItemLink id={l.itemId} name={l.itemName} /></td><td className="right money" style={{ fontWeight: 600, color: l.qty < 0 ? 'var(--danger)' : 'var(--good)' }}>{l.qty > 0 ? '+' : ''}{fmtQty(l.qty)}</td><td>{l.uom}</td><td className="identifier">{l.batch ?? (l.serials?.length ? l.serials.join(', ') : '—')}</td><td className="right money">{fmtMoney(l.rate)}</td><td className="right money">{fmtMoney(l.value)}</td><td style={{ fontSize: 12, color: 'var(--ink-3)' }}>{l.reasonCode ?? a.reasonCode}</td></tr>)}</tbody></table></div> },
        ...
        footer={<>
          ...
          {pending && <Button variant="tinted" tone="danger" disabled={!canAct.ok} reason={canAct.reason} onClick={() => confirm.open({ title: `Reject ${a.number}?`, reasonRequired: true, confirmLabel: 'Reject', danger: true, onConfirm: (r) => engine.actOnApproval(pending.id, 'Reject', { comment: r }) })}>Reject</Button>}
          ...
          {a.status === 'Approved' && <Button variant="primary" tone="good" onClick={() => confirm.open({ title: `Post ${a.number}?`, consequences: [{ engine: 'Stock', text: `${a.lines.length} movement(s) in ${a.warehouseName}` }, { engine: 'Journal', text: `${net >= 0 ? 'Dr Inventory / Cr' : 'Dr'} Inventory adjustments ${fmtMoney(Math.abs(net))}` }], confirmLabel: 'Post adjustment', onConfirm: () => { A.postAdjustment(id); toast.success('Posted'); } })}>Post adjustment</Button>}
