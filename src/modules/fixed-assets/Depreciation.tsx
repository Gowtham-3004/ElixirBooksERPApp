...
export function DepreciationPage() {
  ...
  const cols: Column<DepreciationLine & { id: string }>[] = [
    ...
    { key: 'categoryName', label: 'Category', render: (l) => <span style={{ fontSize: 12, color: '#5F6368' }}>{l.categoryName}</span> },
    { key: 'method', label: 'Method', render: (l) => <span style={{ fontSize: 11, color: '#5F6368' }}>{l.method} {l.ratePct}%</span> },
    ...
    { key: 'depreciation', label: 'Depreciation', align: 'right', render: (l) => <span className="money" style={{ color: '#C0393F' }}>{fmtMoney(l.depreciation, s.currency)}</span>, total: (rs) => <span className="money" style={{ fontWeight: 700, color: '#C0393F' }}>{fmtMoney(rs.reduce((x, l) => x + l.depreciation, 0), s.currency)}</span> },
    ...
    { key: 'note', label: '', render: (l) => (l.note ? <span style={{ fontSize: 11, color: '#8A4B0F' }}>{l.note}</span> : null) },
  ];
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      <div style={{ width: 280, flexShrink: 0, borderRight: '1px solid #EAEAEA', display: 'flex', flexDirection: 'column', background: '#FFF' }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #EAEAEA', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontSize: 13, fontWeight: 600 }}>Depreciation runs</span><Button size="sm" variant={selected ? 'secondary' : 'primary'} onClick={() => setSelected(null)}>New run</Button></div>
        <div style={{ flex: 1, overflow: 'auto' }}>
          {runs.map((r) => (
            <div key={r.id} onClick={() => setSelected(r)} style={{ padding: '12px 16px', borderBottom: '1px solid #F5F5F5', cursor: 'pointer', background: selected?.id === r.id ? '#F2F7FF' : '#FFF', borderLeft: selected?.id === r.id ? '3px solid #325CFF' : '3px solid transparent' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#5F6368' }}><span className="identifier">{r.number} · {r.assetCount} assets</span><span className="money">{fmtMoney(r.total, s.currency)}</span></div>
            </div>
          ))}
          {runs.length === 0 && <div style={{ padding: 16, fontSize: 13, color: '#6E6E71' }}>No runs yet.</div>}
        </div>
      </div>
        ...
        {!view ? (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 12 }}>
              <div><h2 style={{ fontSize: 18, fontWeight: 600 }}>Run depreciation</h2><div style={{ fontSize: 12, color: '#5F6368' }}><ScopeLine extra={`${preview.length} assets · ${fmtMoney(total, s.currency)}`} /></div></div>
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
              <div><h2 style={{ fontSize: 18, fontWeight: 600 }}>{fmtPeriod(view.period)} — depreciation schedule</h2><div style={{ fontSize: 12, color: '#5F6368' }}>{view.number} · <Badge status={view.status} /> · posted {fmtDateTime(view.postedAt)} by {view.postedBy} · journal <span className="identifier link" onClick={() => view.journalId && nav.go(`accounting/journals/${view.journalId}`)}>{view.journalNumber}</span>{view.reversalReason ? ` · ${view.reversalReason}` : ''}</div></div>
...
export function AssetReports() {
  ...
  return (
    <div className="page">
      ...
      {tab === 'register' && <DataTable rows={live} dense columns={[{ key: 'number', label: 'Asset', render: (a) => <div><span className="identifier link">{a.number}</span><div className="cell-secondary">{a.name}</div></div> }, { key: 'categoryName', label: 'Category' }, { key: 'cost', label: 'Gross block', align: 'right', render: (a) => <span className="money">{fmtMoney(a.cost + a.revaluation, s.currency)}</span>, total: (rs) => <span className="money" style={{ fontWeight: 700 }}>{fmtMoney(rs.reduce((x, a) => x + a.cost + a.revaluation, 0), s.currency)}</span> }, { key: 'acc', label: 'Accumulated', align: 'right', render: (a) => <span className="money" style={{ color: '#C0393F' }}>{fmtMoney(accumulated(a) + a.impairment, s.currency)}</span>, total: (rs) => <span className="money" style={{ fontWeight: 700 }}>{fmtMoney(rs.reduce((x, a) => x + accumulated(a) + a.impairment, 0), s.currency)}</span> }, { key: 'nbv', label: 'NBV', align: 'right', render: (a) => <span className="money" style={{ fontWeight: 600 }}>{fmtMoney(nbv(a), s.currency)}</span>, total: (rs) => <span className="money" style={{ fontWeight: 700 }}>{fmtMoney(rs.reduce((x, a) => x + nbv(a), 0), s.currency)}</span> }, { key: 'method', label: 'Method', render: (a) => methodLabel(a) }, { key: 'inServiceDate', label: 'In service', render: (a) => fmtDate(a.inServiceDate) }, { key: 'status', label: 'Status', render: (a) => <Badge status={a.status === 'New' ? 'Submitted' : a.status}>{a.status}</Badge> }]} showTotals onRowClick={(a) => nav.go(`fixed-assets/register/${a.id}`)} />}
      {tab === 'schedule' && <div className="card" style={{ overflow: 'auto' }}><table className="data-table dense"><thead><tr><th>Asset</th>{fyPeriods.map((p) => <th key={p} className="right">{fmtPeriod(p).slice(0, 3)}</th>)}<th className="right">FY total</th></tr></thead><tbody>{schedule.map((r) => <tr key={r.id} style={{ cursor: 'pointer' }} onClick={() => nav.go(`fixed-assets/register/${r.id}?tab=schedule`)}><td><div className="cell-primary">{r.a.name}</div><div className="cell-secondary identifier">{r.a.number}</div></td>{r.months.map((m, i) => <td key={i} className="right money" style={{ color: fyPeriods[i] < (s.state.periodCode ?? '') ? '#0A0A0A' : '#6E6E71' }}>{m ? fmtMoney(m, s.currency) : '—'}</td>)}<td className="right money" style={{ fontWeight: 600 }}>{fmtMoney(r.months.reduce((x, y) => x + y, 0), s.currency)}</td></tr>)}<tr style={{ background: '#F9FBFC', fontWeight: 700 }}><td>Total</td>{fyPeriods.map((_, i) => <td key={i} className="right money">{fmtMoney(schedule.reduce((x, r) => x + r.months[i], 0), s.currency)}</td>)}<td className="right money">{fmtMoney(schedule.reduce((x, r) => x + r.months.reduce((a, b) => a + b, 0), 0), s.currency)}</td></tr></tbody></table><div style={{ padding: '8px 14px', fontSize: 12, color: '#6E6E71' }}>Posted months in black · projected months in grey.</div></div>}
      {tab === 'movement' && <div className="grid-2" style={{ alignItems: 'start' }}>
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}><table className="data-table dense"><tbody>
          {[['Opening gross block', movement.opening], ['Add: additions (capitalized this FY)', movement.additions], ['Less: disposals (cost)', -movement.disposals], ['Closing gross block', movement.opening + movement.additions - movement.disposals], ['Depreciation charged this FY', movement.depreciation], ['Closing NBV (in books)', live.reduce((x, a) => x + nbv(a), 0)]].map(([l, v], i) => <tr key={i} style={{ fontWeight: i === 3 || i === 5 ? 700 : 400, background: i === 3 || i === 5 ? '#F9FBFC' : undefined }}><td>{l}</td><td className="right money" style={{ color: Number(v) < 0 ? '#C0393F' : undefined }}>{fmtMoney(Number(v), s.currency)}</td></tr>)}
        </tbody></table></div>
        <div className="card" style={{ padding: 16 }}><div className="section-title">Reconciliation to ledger</div><div style={{ fontSize: 13 }}>PPE ledger balance (1500): <strong className="money">{fmtMoney(ledgerPpe, s.currency)}</strong> · register gross block (PPE categories): <strong className="money">{fmtMoney(live.filter((a) => a.assetAccountId === IDS.accPPE).reduce((x, a) => x + a.cost + a.revaluation, 0), s.currency)}</strong></div><div style={{ fontSize: 12, color: '#6E6E71', marginTop: 8 }}>Additions this FY: {assets.filter((a) => a.capitalizationDate >= fy.from).map((a) => a.number).join(', ') || 'none'} · Disposals: {assets.filter((a) => a.disposal && a.disposal.date >= fy.from).map((a) => a.number).join(', ') || 'none'}</div></div>
      </div>}
    </div>
  );
}
