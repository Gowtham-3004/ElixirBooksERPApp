        ...
        {!view ? (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end' }}><PeriodPicker value={period} onChange={setPeriod} /><Button variant="secondary" onClick={() => downloadText(`depreciation-preview-${period}.csv`, toCSV(preview as any))} disabled={!preview.length}>Export preview</Button><Button variant="primary" tone="good" onClick={() => setConfirm(true)} disabled={!!dup || !preview.length || !canPost || periodRec?.status === 'Locked'} reason={dup ? `Already posted as ${dup.number}` : !preview.length ? 'Nothing to depreciate' : periodRec?.status === 'Locked' ? 'Period locked' : !canPost ? 'Requires posting permission' : undefined}>Post depreciation</Button></div>
            </div>
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}><Button variant="secondary" onClick={() => downloadText(`${view.number}.csv`, toCSV(view.lines as any))}>Export</Button>{view.status === 'Posted' && !view.reversalOfId && <Button variant="tinted" tone="danger" onClick={() => setReverse(view)} disabled={!canPost}>Reverse run</Button>}</div>
            </div>
