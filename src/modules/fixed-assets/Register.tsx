...
export function AssetRegister() {
  ...
  const cols: Column<Asset>[] = [
    ...
    { key: 'location', label: 'Location', render: (a) => <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{a.location}</span> },
    ...
    { key: 'acc', label: 'Accum. dep.', align: 'right', render: (a) => <span className="money" style={{ color: 'var(--danger)' }}>{fmtMoney(accumulated(a), s.currency)}</span>, total: () => <span className="money" style={{ fontWeight: 600, color: 'var(--danger)' }}>{fmtMoney(totals.acc, s.currency)}</span>, value: (a) => accumulated(a) },
    { key: 'nbv', label: 'WDV / NBV', align: 'right', render: (a) => <span className="money" style={{ fontWeight: 600, color: nbv(a) === 0 ? 'var(--ink-5)' : 'var(--ink)' }}>{fmtMoney(nbv(a), s.currency)}</span>, total: () => <span className="money" style={{ fontWeight: 700 }}>{fmtMoney(totals.nbv, s.currency)}</span>, value: (a) => nbv(a), sortable: true },
    { key: 'method', label: 'Method', render: (a) => <span style={{ fontSize: 11, color: 'var(--ink-3)' }}>{methodLabel(a)}</span> },
    ...
    { id: 'overview', label: 'Overview', content: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div className="grid-4">
          {[['Cost', a.cost], ['Revaluation / (impairment)', a.revaluation - a.impairment], ['Accumulated depreciation', -accumulated(a)], ['Net book value', nbv(a)]].map(([l, v]) => <div key={String(l)} className="kpi-tile"><span className="section-label">{l}</span><div className="money" style={{ fontSize: 20, fontWeight: 600, color: Number(v) < 0 ? 'var(--danger)' : 'var(--ink)' }}>{fmtMoney(Number(v), s.currency)}</div></div>)}
        </div>
        {a.disposal && <div className="card" style={{ padding: 16 }}><div className="section-title">Disposal</div><KV columns={2} items={[{ k: 'Date', v: fmtDate(a.disposal.date) }, { k: 'Buyer', v: a.disposal.buyer ?? '—' }, { k: 'Proceeds', v: fmtMoney(a.disposal.proceeds, s.currency) }, { k: 'NBV at disposal', v: fmtMoney(a.disposal.nbvAtDisposal, s.currency) }, { k: 'Gain / (loss)', v: <span style={{ color: a.disposal.gainLoss >= 0 ? 'var(--good)' : 'var(--danger)', fontWeight: 600 }}>{fmtMoney(a.disposal.gainLoss, s.currency)}</span> }, { k: 'Journal', v: a.disposal.journalNumber ?? '—' }, { k: 'Reason', v: a.disposal.reason }]} /></div>}
        <div className="card" style={{ padding: 16 }}><div className="section-title">Description & notes</div><div style={{ fontSize: 13 }}>{a.description ?? '—'}</div>{a.notes && <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 6 }}>{a.notes}</div>}</div>
      </div>) },
    { id: 'schedule', label: 'Depreciation schedule', content: schedule.length ? <DataTable rows={schedule.map((r) => ({ ...r, id: r.period }))} dense columns={[{ key: 'period', label: 'Period', render: (r) => fmtPeriod(r.period) }, { key: 'openingNbv', label: 'Opening NBV', align: 'right', render: (r) => <span className="money">{fmtMoney(r.openingNbv, s.currency)}</span> }, { key: 'depreciation', label: 'Depreciation', align: 'right', render: (r) => <span className="money" style={{ color: 'var(--danger)' }}>{fmtMoney(r.depreciation, s.currency)}</span>, total: (rs) => <span className="money" style={{ fontWeight: 700 }}>{fmtMoney(rs.reduce((x, r) => x + r.depreciation, 0), s.currency)}</span> }, { key: 'closingNbv', label: 'Closing NBV', align: 'right', render: (r) => <span className="money" style={{ fontWeight: 600 }}>{fmtMoney(r.closingNbv, s.currency)}</span> }]} showTotals /> : <EmptyState compact title="Nothing left to depreciate" description={a.status === 'Disposed' ? 'Asset is disposed.' : 'Net book value has reached residual value.'} /> },
