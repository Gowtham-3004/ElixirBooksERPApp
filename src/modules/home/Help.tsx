      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Card title="Report a problem">
            ...
            <div style={{ fontSize: 12, color: 'var(--ink-4)', margin: '8px 0 12px' }}>We attach your current page ({window.location.hash || '#/home'}), company and a correlation ID. No document data is sent without your consent.</div>
            ...
            {mine.length > 0 && (
              <div style={{ marginTop: 16 }}>
                ...
                {mine.map((j) => (
                  <div key={j.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--hairline)', fontSize: 13 }}>
                    <span>{j.name.replace('Support ticket · ', '')} <span className="identifier" style={{ fontSize: 11, color: 'var(--ink-4)' }}>{j.correlationId}</span></span>
                    <span style={{ display: 'flex', gap: 8, alignItems: 'center' }}><span style={{ fontSize: 12, color: 'var(--ink-4)' }}>{fmtDateTime(j.createdAt)}</span><Badge status={j.status} /></span>
                  </div>
                ))}
              </div>
            )}
          ...
          <Card title="Keyboard shortcuts">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 13 }}>
              {[[`${mod} K`, 'Global search'], ['?', 'Show shortcuts'], ['Esc', 'Close drawer / dialog'], ['↑ ↓ Enter', 'Move and pick in lists'], ['Tab', 'Next field in a line grid']].map(([k, v]) => (
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ color: 'var(--ink-3)' }}>{v}</span><span style={{ display: 'flex', gap: 4 }}>{k.split(' ').map((x, i) => <Kbd key={i}>{x}</Kbd>)}</span></div>
              ))}
            </div>
          </Card>
          <Card title="About">
