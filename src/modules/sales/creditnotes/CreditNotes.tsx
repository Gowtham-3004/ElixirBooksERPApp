...
export function CreditNoteForm({ id, invoiceId }: { id?: string; invoiceId?: string }) {
  ...
  return (
    <div className="page">
      ...
      <section>
        <div className="section-title">Credited lines <span style={{ fontSize: 12, fontWeight: 400, color: '#5F6368' }}>· quantity capped at the returnable balance of each invoice line</span></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <Card title="Projected journal" padding={0}><table className="data-table dense"><thead><tr><th>Account</th><th className="right">Dr</th><th className="right">Cr</th></tr></thead><tbody>{creditNoteJournalLines(doc).map((l, i) => { const a = db.find<any>(C.accounts, l.accountId); return <tr key={i}><td><span className="identifier">{a?.code}</span> · {a?.name}{l.partyName ? <span style={{ color: '#5F6368' }}> · {l.partyName}</span> : null}</td><td className="right money">{l.dr ? fmtMoney(l.dr, doc.currency) : '—'}</td><td className="right money">{l.cr ? fmtMoney(l.cr, doc.currency) : '—'}</td></tr>; })}</tbody></table></Card>
        </div>
        <div style={{ position: 'sticky', top: 16 }}><div className="section-title">Totals</div><TotalsLadder totals={doc.totals} currency={doc.currency} baseCurrency={s.currency} rate={doc.rate} showPaid={false} /><div style={{ fontSize: 12, color: '#5F6368', marginTop: 8 }}>{fmtMoney(Math.min(doc.totals.total, inv.totals.due), doc.currency)} will settle against {inv.number}{doc.totals.total > inv.totals.due ? `; ${fmtMoney(doc.totals.total - inv.totals.due, doc.currency)} becomes unapplied credit` : ''}.</div><div style={{ marginTop: 16 }}><div className="section-title">Tax breakup</div><TaxBreakup totals={doc.totals} currency={doc.currency} /></div></div>
      </div>
      <FormFooter savedAt={draft.savedAt} dirty={draft.dirty} conflict={draft.conflict} onReload={draft.reload} left={errors.length ? <span style={{ color: '#C0393F' }}>{errors.length} issue(s)</span> : undefined}>
        ...
        {wf ? <Button variant="primary" onClick={submit} data-testid="submit-credit-note">Submit for approval</Button> : <Button variant="primary" onClick={() => { if (errors.length) { draft.setErrors(errors.map((m) => ({ message: m }))); toast.error('Fix the highlighted issues'); return; } setConfirm(true); }} data-testid="post-credit-note">Post credit note</Button>}
      </FormFooter>
