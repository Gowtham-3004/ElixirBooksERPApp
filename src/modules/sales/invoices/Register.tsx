...
export default function InvoiceRegister() {
  ...
  const columns: Column<SalesInvoice>[] = [
    ...
    { key: 'due', label: 'Due', align: 'right', render: (r) => (r.status === 'Posted' || r.status === 'Settled') ? <Money value={r.totals.due} currency={r.currency} /> : <span style={{ color: '#B0B5BF' }}>—</span>, value: (r) => r.totals.due, total: (rs) => <span className="money" style={{ fontWeight: 600 }}>{fmtMoney(rs.filter((x) => x.currency === s.currency).reduce((a, x) => a + (x.status === 'Posted' ? x.totals.due : 0), 0), s.currency)}</span> },
    { key: 'dueDate', label: 'Due date', sortable: true, render: (r) => r.dueDate && r.status !== 'Draft' && r.status !== 'Cancelled' ? <div><div>{fmtDate(r.dueDate)}</div>{isOverdue(r) && <span className="badge badge-overdue" style={{ marginTop: 2 }}>Overdue {daysBetween(r.dueDate, today())}d</span>}</div> : <span style={{ color: '#B0B5BF' }}>—</span>, value: (r) => r.dueDate },
  ];
