      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <table className="data-table dense">
          ...
          <tbody>
            ...
            {st!.lines.map((l, i) => <tr key={i} className="clickable" onClick={() => nav.go(l.link)}><td>{fmtDate(l.date)}</td><td>{l.type}{l.status === 'Reversed' ? <> <Badge status="Reversed" /></> : null}</td><td className="identifier link">{l.number}</td><td style={{ color: 'var(--ink-3)' }}>{l.ref ?? '—'}</td><td className="right money">{l.debit ? fmtMoney(l.debit, cur) : '—'}</td><td className="right money">{l.credit ? fmtMoney(l.credit, cur) : '—'}</td><td className="right money" style={{ fontWeight: 500 }}>{fmtMoney(l.balance, cur)}</td></tr>)}
            {st!.lines.length === 0 && <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--ink-3)', height: 56 }}>No transactions in this period</td></tr>}
          </tbody>
