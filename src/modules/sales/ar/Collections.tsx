  ...
  return (
    <div className="page">
      ...
      <div className="grid-4">
        ...
        <KpiTile label="Overdue" value={<span style={{ color: '#C0393F' }}>{fmtMoney(totOverdue, cur)}</span>} sub={totOut ? `${Math.round((totOverdue / totOut) * 100)}% of outstanding` : undefined} />
        ...
        <KpiTile label="Broken promises" value={<span style={{ color: brokenPromises.length ? '#C0393F' : undefined }}>{brokenPromises.length}</span>} sub="promise date passed, still open" />
      </div>
        ...
        return (
          <div key={r.customer.id} className="card" style={{ overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 16px', cursor: 'pointer' }} onClick={() => setExpanded(open ? null : r.customer.id)}>
              <div style={{ flex: 1.4, minWidth: 0 }}><div style={{ fontWeight: 600 }}>{r.customer.name}</div><div style={{ fontSize: 12, color: '#5F6368' }}>{r.customer.code} · {r.customer.paymentTerms} · {r.items.length} open invoice{r.items.length === 1 ? '' : 's'}{r.lastActivity ? ` · last: ${r.lastActivity.type} ${fmtDate(r.lastActivity.createdAt)}` : ''}</div></div>
              ...
              <div style={{ width: 150, textAlign: 'right' }}><div className="section-label">Overdue</div><div className="money" style={{ fontWeight: 600, color: r.overdue ? '#C0393F' : '#5F6368' }}>{fmtMoney(r.overdue, cur)}</div>{r.oldest > 0 && <Pill tone={r.oldest > 60 ? 'critical' : 'warning'}>oldest {r.oldest} d</Pill>}</div>
              <div style={{ width: 190 }}><div className="section-label">Credit exposure</div><Meter value={r.outstanding - r.credits} max={limit || r.outstanding} /><div style={{ fontSize: 11, color: (r.outstanding - r.credits) > limit && limit > 0 ? '#C0393F' : '#6E6E71' }}>{limit ? `${fmtMoney(r.outstanding - r.credits, cur)} of ${fmtMoney(limit, cur)}` : 'No limit set'}{r.credits ? ` · ${fmtMoney(r.credits, cur)} credit` : ''}</div></div>
              <div style={{ width: 170 }}>{r.promise ? <div style={{ fontSize: 12 }}><Badge status={r.promise.promiseDate && r.promise.promiseDate < today() ? 'Overdue' : 'Pending'}>{r.promise.promiseDate && r.promise.promiseDate < today() ? 'Promise broken' : 'Promise to pay'}</Badge><div style={{ color: '#5F6368', marginTop: 2 }}>{fmtMoney(r.promise.promiseAmount ?? 0, cur)} by {fmtDate(r.promise.promiseDate)}</div></div> : r.openFollowUps ? <Badge status="Submitted">{r.openFollowUps} follow-up{r.openFollowUps === 1 ? '' : 's'}</Badge> : <span style={{ fontSize: 12, color: '#B0B5BF' }}>No promise</span>}</div>
              <span style={{ color: '#B0B5BF' }}>{open ? '▾' : '›'}</span>
            </div>
            {open && (
              <div style={{ borderTop: '1px solid #EFEFEF', padding: 16, display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 20 }}>
                <div>
