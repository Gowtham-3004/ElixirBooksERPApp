...
export default function ArAgeing() {
  ...
  const columns: Column<AgeingRow>[] = [
    ...
    { key: 'current', label: 'Current', align: 'right', render: (r) => <span className="money" style={{ color: r.current ? 'var(--good)' : 'var(--ink-5)' }}>{r.current ? fmtMoney(r.current, cur) : '—'}</span>, value: (r) => r.current, total: () => <span className="money">{fmtMoney(tot('current'), cur)}</span> },
    { key: 'd030', label: '1–30 days', align: 'right', render: (r) => <span className="money" style={{ color: r.d030 ? '#F97316' : 'var(--ink-5)' }}>{r.d030 ? fmtMoney(r.d030, cur) : '—'}</span>, value: (r) => r.d030, total: () => <span className="money">{fmtMoney(tot('d030'), cur)}</span> },
    { key: 'd3160', label: '31–60 days', align: 'right', render: (r) => <span className="money" style={{ color: r.d3160 ? '#EF8C1E' : 'var(--ink-5)' }}>{r.d3160 ? fmtMoney(r.d3160, cur) : '—'}</span>, value: (r) => r.d3160, total: () => <span className="money">{fmtMoney(tot('d3160'), cur)}</span> },
    { key: 'd6190', label: '61–90 days', align: 'right', render: (r) => <span className="money" style={{ color: r.d6190 ? '#E07014' : 'var(--ink-5)' }}>{r.d6190 ? fmtMoney(r.d6190, cur) : '—'}</span>, value: (r) => r.d6190, total: () => <span className="money">{fmtMoney(tot('d6190'), cur)}</span> },
    { key: 'd90p', label: '> 90 days', align: 'right', render: (r) => <span className="money" style={{ color: r.d90p ? 'var(--danger)' : 'var(--ink-5)' }}>{r.d90p ? fmtMoney(r.d90p, cur) : '—'}</span>, value: (r) => r.d90p, total: () => <span className="money">{fmtMoney(tot('d90p'), cur)}</span> },
    ...
    { key: 'credits', label: 'Unapplied credits', align: 'right', render: (r) => <span className="money" style={{ color: r.credits ? 'var(--good)' : 'var(--ink-5)' }}>{r.credits ? `−${fmtMoney(r.credits, cur)}` : '—'}</span>, value: (r) => r.credits, total: () => <span className="money">−{fmtMoney(tot('credits'), cur)}</span> },
    { key: 'overdue', label: 'Overdue', align: 'right', sortable: true, render: (r) => (r.overdue > 0 ? <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}><span className="money" style={{ color: 'var(--danger)', fontWeight: 600 }}>{fmtMoney(r.overdue, cur)}</span><Pill tone="critical">{r.oldestDays} d</Pill></div> : <span style={{ color: 'var(--ink-5)' }}>—</span>), value: (r) => r.overdue, total: () => <span className="money" style={{ color: 'var(--danger)', fontWeight: 600 }}>{fmtMoney(tot('overdue'), cur)}</span> },
    { key: 'limit', label: 'Credit limit', align: 'right', render: (r) => (r.creditLimit ? <span className="money" style={{ color: r.net > r.creditLimit ? 'var(--danger)' : 'var(--ink-3)' }}>{fmtMoney(r.creditLimit, cur)}</span> : '—'), value: (r) => r.creditLimit },
  ];
  const buckets = [{ label: 'Current', val: tot('current'), color: 'var(--good)' }, { label: '1–30 days', val: tot('d030'), color: '#F97316' }, { label: '31–60 days', val: tot('d3160'), color: '#EF8C1E' }, { label: '61–90 days', val: tot('d6190'), color: '#E07014' }, { label: '> 90 days', val: tot('d90p'), color: 'var(--danger)' }, { label: 'Total outstanding', val: tot('total'), color: 'var(--ink)' }];
  return (
    <div className="page">
      ...
      <div style={{ fontSize: 12, color: 'var(--ink-4)' }}>Buckets are by due date as at {asAt}; settlements after that date are added back so the report is point-in-time. Base currency {cur}.</div>
    </div>
  );
}
