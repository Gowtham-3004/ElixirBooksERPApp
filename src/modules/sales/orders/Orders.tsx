...
export function OrderForm({ id }: { id?: string }) {
  ...
  return (
    <div className="page">
      ...
      <section>
        <div className="section-title">Lines {maxDisc > settings.salesDiscountThresholdPct && <span style={{ fontSize: 12, fontWeight: 400, color: 'var(--warn)' }}>· discount above {settings.salesDiscountThresholdPct}% needs sales.order.discount</span>}</div>
      ...
      <FormFooter savedAt={draft.savedAt} dirty={draft.dirty} conflict={draft.conflict} onReload={draft.reload} left={v.errors.length ? <span style={{ color: 'var(--danger)' }}>{v.errors.length} issue(s)</span> : undefined}>
        ...
        <Button variant="primary" tone="good" onClick={submit} disabled={!!v.discountBlocked || (v.credit ? !v.credit.ok : false)} reason={v.discountBlocked ?? (v.credit && !v.credit.ok ? v.credit.message : undefined)} data-testid="submit-order">Submit order</Button>
      </FormFooter>
    </div>
  );
}
    ...
    <>
      <DocumentPage backLabel="Sales orders" onBack={() => nav.go('sales/orders')} number={so.number} badges={<><Badge status={so.status} />{so.creditCheck?.message && <Badge status="Returned">Credit exception</Badge>}</>}
        ...
        rail={<SalesRail doc={so}>
          <RailSection label="Fulfilment"><div style={{ fontSize: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>{[['Reserved', q.reserved], ['Delivered', q.delivered], ['Invoiced', q.invoiced]].map(([k, v]) => <div key={String(k)}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span>{k}</span><span className="money">{fmtQty(v as number)} / {fmtQty(q.ordered)}</span></div><Meter value={v as number} max={q.ordered} tone={(v as number) >= q.ordered && q.ordered > 0 ? 'good' : undefined} /></div>)}<div style={{ color: q.pending > 0 ? 'var(--warn)' : 'var(--good)' }}>Pending {fmtQty(q.pending)} · {fmtMoney(q.pendingValue, so.currency)}</div>{so.reservationExpiry && q.reserved > 0 && <div style={{ color: 'var(--ink-4)' }}>Reservations expire {fmtDate(so.reservationExpiry)}</div>}</div></RailSection>
          {so.creditCheck && <RailSection label="Credit check"><div style={{ fontSize: 12, color: 'var(--ink-3)' }}>{so.creditCheck.mode} · exposure {fmtMoney(so.creditCheck.exposure)} / limit {fmtMoney(so.creditCheck.limit)}{so.creditCheck.message ? <div style={{ color: 'var(--warn)' }}>{so.creditCheck.message}</div> : null}</div></RailSection>}
        </SalesRail>}
