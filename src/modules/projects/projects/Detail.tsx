...
export default function ProjectDetail({ id, tab, onTab }: { id: string; tab?: string; onTab?: (t: string) => void }) {
  ...
  const invCols: Column<DocHeader>[] = [
    ...
    { key: 'due', label: 'Due', align: 'right', render: (i) => i.status === 'Posted' ? <Money value={i.totals?.due ?? 0} currency={i.currency} /> : <span style={{ color: 'var(--ink-5)' }}>—</span> },
  ];
