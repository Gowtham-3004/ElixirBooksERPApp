...
function RetainerRegister({ customerId }: { customerId?: string }) {
  ...
  const cols: Column<Retainer>[] = [
    ...
    { key: 'remaining', label: 'Remaining', align: 'right', render: (r) => <span className="money" style={{ fontWeight: 600, color: r.remaining > 0 ? '#12784E' : '#5F6368' }}>{fmtMoney(r.remaining, r.currency)}</span>, value: (r) => r.remaining, total: (rs) => <span className="money" style={{ fontWeight: 600 }}>{fmtMoney(rs.filter((x) => x.currency === s.currency).reduce((a, x) => a + x.remaining, 0), s.currency)}</span> },
