  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0,1fr))', gap: 12 }}>
        ...
        <KpiTile label={`Billed ${fmtPeriod(period)}`} value={fmtMoney(data.billed, s.currency)} sub={data.drafts.length ? `${data.drafts.length} draft invoice(s) awaiting posting` : 'All generated invoices posted'} deltaTone="neutral" meta="Posted contract invoices · excl. tax" onClick={() => nav.go('sales/invoices')} />
        <KpiTile label="Deferred revenue" value={fmtMoney(data.deferred, s.currency)} sub={<>Unbilled (accrued) {fmtMoney(data.unbilledGl, s.currency)}</>} meta={`GL 2400 / 1160 balances · ${fmtDate(today())}`} onClick={() => nav.go('projects/revenue')} />
