...
export function ExpensesRegister() {
  ...
  const cols: Column<ExpenseClaim>[] = [
    ...
    { key: 'category', label: 'Category', render: (c) => <span style={{ fontSize: 12, color: '#5F6368' }}>{Array.from(new Set(c.lines.map((l) => l.categoryName))).join(', ')}</span> },
    { key: 'purpose', label: 'Description', render: (c) => <span style={{ fontSize: 12, color: '#5F6368' }}>{c.purpose}</span> },
    { key: 'amount', label: 'Amount', align: 'right', render: (c) => <span className="money">{fmtMoney(c.totals.amount, s.currency)}</span> },
    { key: 'tax', label: 'Tax', align: 'right', render: (c) => <span className="money" style={{ color: '#5F6368' }}>{c.totals.tax ? fmtMoney(c.totals.tax, s.currency) : '—'}</span> },
    ...
    { key: 'reimb', label: 'Reimbursed', render: (c) => c.paymentMethod === 'Corporate card' ? <span style={{ fontSize: 11, color: '#B0B5BF' }}>N/A</span> : c.status === 'Reimbursed' ? <Badge status="Posted">Yes</Badge> : c.status === 'Approved' ? <Badge status="Returned">Pending</Badge> : <span style={{ color: '#B0B5BF' }}>—</span> },
  ];
  return (
  ...
  return (
    <div className="page">
          ...
          <tbody>
            {c.lines.map((l) => { const cat = cats.find((x) => x.id === l.categoryId); const needsReceipt = (cat?.receiptRequired || l.total > st.receiptRequiredAbove) && !l.hasReceipt; return (
              <tr key={l.id} className={needsReceipt || !l.categoryId || l.amount <= 0 ? 'error-row' : ''}>
                ...
                <td><select className="field-input grid" value={l.categoryId} onChange={(e) => pickCat(l.id, e.target.value)}><option value="">— Category —</option>{cats.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select>{cat && <div style={{ fontSize: 10, color: '#6E6E71' }}>{db.find<any>(C.accounts, cat.accountId)?.code} · {db.find<any>(C.accounts, cat.accountId)?.name}</div>}</td>
                ...
                <td><select className="field-input grid" value={l.taxRateId ?? ''} onChange={(e) => updLine(l.id, { taxRateId: e.target.value || undefined })}><option value="">No GST</option>{taxOpts.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}</select>{l.tax > 0 && <div style={{ fontSize: 10, color: '#6E6E71' }}>{fmtMoney(l.tax, s.currency)} input credit</div>}</td>
                ...
                <td><CheckboxField checked={!!l.hasReceipt} onChange={(v) => updLine(l.id, { hasReceipt: v })} label={<span style={{ fontSize: 11 }}>{needsReceipt ? <span style={{ color: '#C0393F' }}>required</span> : 'attached'}</span>} /></td>
      ...
      <div className="grid-2" style={{ alignItems: 'start' }}>
        <div><div className="section-title">Receipts & attachments</div>{id ? <AttachmentsPanel objectType="Expense Claim" objectId={c.id} /> : <div style={{ fontSize: 12, color: '#6E6E71' }}>Save the draft to attach receipt files; tick “receipt” on each line for now.</div>}<TextArea label="Notes" value={c.notes} onChange={(v) => upd({ notes: v })} rows={2} style={{ marginTop: 12 }} /></div>
        <div className="summary-block"><div className="ladder-row"><span className="ladder-label">Amount</span><span className="ladder-value">{fmtMoney(c.totals.amount, s.currency)}</span></div><div className="ladder-row"><span className="ladder-label">GST (input credit)</span><span className="ladder-value">{fmtMoney(c.totals.tax, s.currency)}</span></div><div style={{ height: 1, background: '#DADCE0', margin: '6px 0' }} /><div className="ladder-row"><span className="ladder-label" style={{ fontWeight: 600, color: '#0A0A0A' }}>Total claim</span><span className="ladder-value" style={{ fontWeight: 600, fontSize: 16 }}>{fmtMoney(c.totals.total, s.currency)}</span></div><div style={{ fontSize: 12, color: '#6E6E71', marginTop: 8 }}>{c.totals.total > 5000 ? 'Above ₹5,000 → manager approval (WF-003)' : 'At or below ₹5,000 → auto-approved and posted on submit'} · {c.paymentMethod === 'Personal' ? 'credits employee payable (2340) for reimbursement' : 'credits the corporate card liability'}</div></div>
      </div>
  ...
  return (
    <>
      ...
      <Modal open={confirm === 'reimburse'} onClose={() => setConfirm(null)} title={`Reimburse ${fmtMoney(c.totals.total, s.currency)} to ${c.employeeName}`} description="Bank: posts Dr employee payable · Cr bank and settles the open item. Payroll: adds the amount to this month's payroll inputs (settled when the run posts)." footer={<><Button variant="secondary" onClick={() => setConfirm(null)}>Cancel</Button><Button variant="primary" onClick={() => { act(() => reimburseClaim(c, reimb), reimb.mode === 'Bank' ? 'Reimbursement posted' : 'Queued for payroll'); setConfirm(null); }}>{reimb.mode === 'Bank' ? 'Record payment' : 'Push to payroll'}</Button></>}>
        ...
        <div style={{ fontSize: 12, color: '#6E6E71', marginTop: 8 }}>Bank reimbursements are also visible under Banking as payments to employees.</div>
      </Modal>
    </>
  );
}
