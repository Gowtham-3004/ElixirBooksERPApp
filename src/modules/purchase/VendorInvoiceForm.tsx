...
function Inner({ existing, poId, grnId, supplierId }: { existing?: VendorInvoice; poId?: string; grnId?: string; supplierId?: string }) {
  ...
  return (
    <div className="page">
      ...
      <LineItemGrid lines={v.lines} onChange={(lines) => set({ lines: lines as VendorInvoiceLine[] })} direction="purchase" partyId={v.partyId} currency={v.currency} showWarehouse={!v.poId} showDiscount showTax showAccount={!v.poId} totals={v.totals} sourceLinked={!!v.poId}
        extraColumns={v.poId ? [{ key: 'po', label: 'PO rate · qty', width: 120, render: (l) => { const x = l as VendorInvoiceLine; const diff = x.poRate !== undefined && x.rate !== x.poRate; return <span style={{ fontSize: 12, color: diff ? '#C0393F' : '#5F6368' }}>{fmtMoney(x.poRate ?? 0, v.currency)} · {fmtQty(x.poQty ?? 0)}{diff && <div>Δ {fmtMoney((x.rate - (x.poRate ?? 0)) * x.qty, v.currency)}</div>}</span>; } }, { key: 'grn', label: 'GRN accepted', width: 100, render: (l) => <span style={{ fontSize: 12, color: '#5F6368' }}>{(l as VendorInvoiceLine).grnQty !== undefined ? fmtQty((l as VendorInvoiceLine).grnQty!) : '—'}</span> }] : undefined} />
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 380px', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card" style={{ padding: 16 }}><div className="section-title">Totals</div><TotalsLadder totals={v.totals} currency={v.currency} baseCurrency={s.currency} rate={v.rate} showPaid={false} />{v.reverseCharge && <div style={{ fontSize: 12, color: '#8A4B0F', marginTop: 8 }}>Reverse charge: tax of {fmtMoney(v.lines.reduce((x, l) => x + (l.reverseCharge ? l.taxAmt : 0), 0), v.currency)} is self-assessed (Dr input / Cr output) and not added to the payable.</div>}</div>
