  ...
  return (
    <RegisterPage<PaymentBatch> title="Payment batches" subtitle={`${mine.length} batches · maker-checker · ${s.company?.tradeName}`} entity="payment batches" rows={mine} searchKeys={['number', 'makerName', 'bankAccountName']}
      columns={[
        ...
        { key: 'results', label: 'Results', render: (r) => { const a = r.lines.filter((l) => l.status === 'Accepted' || l.status === 'Completed').length; const f = r.lines.filter((l) => l.status === 'Failed').length; return <span style={{ fontSize: 12 }}>{a ? <span style={{ color: '#12784E' }}>{a} ok</span> : null}{a && f ? ' · ' : ''}{f ? <span style={{ color: '#C0393F' }}>{f} failed</span> : null}{!a && !f ? '—' : ''}</span>; } },
    ...
    <>
      <div className="page">
        ...
        <div className="card" style={{ padding: '12px 20px', display: 'flex', gap: 0, alignItems: 'center' }}>
          {STEPS.map((st, i) => <div key={st} style={{ display: 'flex', alignItems: 'center', flex: i < STEPS.length - 1 ? 1 : undefined }}><div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: i <= stepIdx ? '#0A0A0A' : '#B0B5BF', fontWeight: i === stepIdx ? 600 : 400 }}><span style={{ width: 20, height: 20, borderRadius: '50%', background: i < stepIdx ? '#12784E' : i === stepIdx ? '#325CFF' : '#F3F5F5', color: i <= stepIdx ? '#fff' : '#5F6368', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 10 }}>{i < stepIdx ? '✓' : i + 1}</span>{st}</div>{i < STEPS.length - 1 && <div style={{ flex: 1, height: 1, background: i < stepIdx ? '#12784E' : '#EAEAEA', margin: '0 10px' }} />}</div>)}
              ...
              <tbody>{b.lines.map((l) => {
                ...
                return (
                  <tr key={l.id} className={l.status === 'Failed' ? 'error-row' : ''}>
                    ...
                    <td style={{ fontSize: 12 }}>{l.docNumbers.map((n, i) => <DocLink key={n} path={`purchase/vendor-invoices/${db.find<OpenItem>(C.openItems, l.openItemIds[i])?.docId ?? ''}`} number={n} />).reduce<any[]>((acc, el, i) => (i ? [...acc, ', ', el] : [el]), [])}<div style={{ color: '#5F6368' }}>outstanding {fmtMoney(outstanding)}</div></td>
                    <td style={{ fontSize: 12 }}>{l.accountNumber ? <>{l.bankName} · <MaskedValue value={l.accountNumber} canReveal={s.can('purchase.batch.approve') || s.can('banking.*')} onReveal={() => engine.audit({ action: 'bank_detail.revealed', objectType: 'Payment Batch', objectId: b.id, objectNumber: b.number, detail: l.supplierName, sensitive: true })} /> · {l.ifsc}</> : <span style={{ color: '#C0393F' }}>No bank details</span>}</td>
                    ...
                    <td style={{ fontSize: 12, color: '#5F6368' }}>{l.note ?? '—'}</td>
                    <td>{canResult ? <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}><TextField size="sm" value={results[l.id] ?? ''} onChange={(v) => setResults({ ...results, [l.id]: v })} placeholder="UTR or bank error" /><Button size="sm" variant="primary" onClick={() => run(() => A.setBatchLineResult(id, l.id, 'Accepted', results[l.id] ?? ''), 'Accepted')}>Accepted</Button><Button size="sm" variant="danger" onClick={() => run(() => A.setBatchLineResult(id, l.id, 'Failed', results[l.id] ?? ''), 'Marked failed')}>Failed</Button></div> : l.utr ? <span className="identifier" style={{ fontSize: 12 }}>{l.utr}</span> : l.error ? <span style={{ fontSize: 12, color: '#C0393F' }}>{l.error}</span> : '—'}</td>
