              ...
              <tbody>{b.lines.map((l) => {
                ...
                return (
                  <tr key={l.id} className={l.status === 'Failed' ? 'error-row' : ''}>
                    ...
                    <td>{canResult ? <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}><TextField size="sm" value={results[l.id] ?? ''} onChange={(v) => setResults({ ...results, [l.id]: v })} placeholder="UTR or bank error" /><Button size="sm" variant="primary" onClick={() => run(() => A.setBatchLineResult(id, l.id, 'Accepted', results[l.id] ?? ''), 'Accepted')}>Accepted</Button><Button size="sm" variant="danger" onClick={() => run(() => A.setBatchLineResult(id, l.id, 'Failed', results[l.id] ?? ''), 'Marked failed')}>Failed</Button></div> : l.utr ? <span className="identifier" style={{ fontSize: 12 }}>{l.utr}</span> : l.error ? <span style={{ fontSize: 12, color: 'var(--danger)' }}>{l.error}</span> : '—'}</td>
