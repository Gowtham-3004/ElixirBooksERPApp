...
import { RegisterPage, DocumentPage, Button, Badge, TwoLine, DateField, TextField, NumberField, ActivityTab, AccountingTab, AttachmentsPanel, RailSection, useToast, Banner, EmptyState, PeriodBanner, SummaryBlock, ActionMenu, type Column } from '../../components/ui';
...
export function GrnRegister() {
  ...
  const columns: Column<Grn>[] = [
    ...
    { key: 'accepted', label: 'Accepted', align: 'right', value: (r) => sum(r, 'acceptedQty'), render: (r) => <span className="money" style={{ color: '#12784E' }}>{fmtQty(sum(r, 'acceptedQty'))}</span> },
    { key: 'rejected', label: 'Rejected', align: 'right', value: (r) => sum(r, 'rejectedQty'), render: (r) => { const n = sum(r, 'rejectedQty'); return <span className="money" style={{ color: n > 0 ? '#C0393F' : '#B0B5BF' }}>{n > 0 ? fmtQty(n) : '—'}</span>; } },
...
export function GrnForm({ id, poId }: { id?: string; poId?: string }) {
  ...
  if (!existing && !pickPo) {
    ...
    return (
      <div className="page">
        <div className="page-header"><div><button type="button" className="btn-link" style={{ color: '#5F6368' }} onClick={() => nav.go('purchase/grn')}>← Goods receipts</button><h1 className="page-title">New goods receipt</h1><div className="page-subtitle">Choose the approved purchase order you are receiving against</div></div></div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="data-table dense">
            ...
            <tbody>
              {g.lines.map((l, i) => {
                ...
                return (
                  <tr key={l.id} className={over || mismatch ? 'error-row' : ''}>
                    ...
                    <td>{item?.tracking === 'Batch' ? <input className="field-input grid" placeholder="Batch no. *" value={l.batch ?? ''} onChange={(e) => upd(l.id, { batch: e.target.value })} /> : item?.tracking === 'Serial' ? <input className="field-input grid" placeholder={`${l.acceptedQty} serials, comma-separated`} value={(l.serials ?? []).join(',')} onChange={(e) => upd(l.id, { serials: e.target.value.split(',').map((x) => x.trim()).filter(Boolean) })} /> : <span style={{ color: '#B0B5BF', fontSize: 12 }}>n/a</span>}</td>
                    <td>{item?.tracking === 'Batch' ? <input type="date" className="field-input grid" value={l.expiryDate ?? ''} onChange={(e) => upd(l.id, { expiryDate: e.target.value || undefined })} /> : <span style={{ color: '#B0B5BF', fontSize: 12 }}>—</span>}</td>
              ...
              {g.lines.length === 0 && <tr><td colSpan={14} style={{ textAlign: 'center', color: '#5F6368', height: 64 }}>Nothing left to receive on this PO.</td></tr>}
            </tbody>
          </table>
        </div>
        <div style={{ display: 'flex', gap: 24, padding: '10px 16px', borderTop: '1px solid #EAEAEA', background: '#F9FBFC', fontSize: 12, color: '#5F6368' }}>
          <span>Received <strong>{fmtQty(totals.received)}</strong></span><span>Accepted <strong style={{ color: '#12784E' }}>{fmtQty(totals.accepted)}</strong></span><span>Rejected <strong style={{ color: '#C0393F' }}>{fmtQty(totals.rejected)}</strong></span><span>Held (QC-HOLD) <strong style={{ color: '#8A4B0F' }}>{fmtQty(totals.held)}</strong></span>
          ...
          { id: 'lines', label: 'Lines & QC', content: (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div className="card" style={{ overflow: 'auto' }}><table className="data-table dense"><thead><tr><th>#</th><th>Item</th><th className="right">Ordered</th><th className="right">Received</th><th className="right">Accepted</th><th className="right">Rejected</th><th className="right">Held</th><th>Disposition</th><th>Warehouse · bin</th><th>Batch / serial</th><th className="right">Rate</th><th className="right">Value</th><th className="right">Invoiced</th><th>QC note</th></tr></thead>
                <tbody>{g.lines.map((l, i) => <tr key={l.id}><td>{i + 1}</td><td><ItemLink id={l.itemId} name={l.itemName} /></td><td className="right money">{fmtQty(l.orderedQty)}</td><td className="right money">{fmtQty(l.receivedQty)}</td><td className="right money" style={{ color: '#12784E' }}>{fmtQty(l.acceptedQty)}</td><td className="right money" style={{ color: l.rejectedQty ? '#C0393F' : '#B0B5BF' }}>{l.rejectedQty ? fmtQty(l.rejectedQty) : '—'}</td><td className="right money">{l.heldQty ? fmtQty(l.heldQty) : '—'}</td><td>{l.disposition ?? '—'}</td><td style={{ fontSize: 12 }}>{whName(l.warehouseId)}{l.bin ? ` · ${l.bin}` : ''}</td><td className="identifier" style={{ fontSize: 12 }}>{l.batch ?? (l.serials?.length ? `${l.serials.length} serials` : '—')}{l.expiryDate ? ` · exp ${fmtDate(l.expiryDate)}` : ''}</td><td className="right money">{fmtMoney(l.rate, g.currency)}</td><td className="right money">{fmtMoney(l.taxable, g.currency)}</td><td className="right money">{fmtQty(l.invoicedQty ?? 0)}</td><td style={{ fontSize: 12, color: '#5F6368' }}>{l.qcNote ?? '—'}</td></tr>)}</tbody></table></div>
            </div>) },
