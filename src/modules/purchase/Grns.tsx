              ...
              {g.lines.map((l, i) => {
                ...
                return (
                  <tr key={l.id} className={over || mismatch ? 'error-row' : ''}>
                    ...
                    <td>{item?.tracking === 'Batch' ? <input className="field-input grid" placeholder="Batch no. *" value={l.batch ?? ''} onChange={(e) => upd(l.id, { batch: e.target.value })} /> : item?.tracking === 'Serial' ? <input className="field-input grid" placeholder={`${l.acceptedQty} serials, comma-separated`} value={(l.serials ?? []).join(',')} onChange={(e) => upd(l.id, { serials: e.target.value.split(',').map((x) => x.trim()).filter(Boolean) })} /> : <span style={{ color: 'var(--ink-5)', fontSize: 12 }}>n/a</span>}</td>
                    <td>{item?.tracking === 'Batch' ? <input type="date" className="field-input grid" value={l.expiryDate ?? ''} onChange={(e) => upd(l.id, { expiryDate: e.target.value || undefined })} /> : <span style={{ color: 'var(--ink-5)', fontSize: 12 }}>—</span>}</td>
          ...
          { id: 'lines', label: 'Lines & QC', content: (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div className="card" style={{ overflow: 'auto' }}><table className="data-table dense"><thead><tr><th>#</th><th>Item</th><th className="right">Ordered</th><th className="right">Received</th><th className="right">Accepted</th><th className="right">Rejected</th><th className="right">Held</th><th>Disposition</th><th>Warehouse · bin</th><th>Batch / serial</th><th className="right">Rate</th><th className="right">Value</th><th className="right">Invoiced</th><th>QC note</th></tr></thead>
                <tbody>{g.lines.map((l, i) => <tr key={l.id}><td>{i + 1}</td><td><ItemLink id={l.itemId} name={l.itemName} /></td><td className="right money">{fmtQty(l.orderedQty)}</td><td className="right money">{fmtQty(l.receivedQty)}</td><td className="right money" style={{ color: 'var(--good)' }}>{fmtQty(l.acceptedQty)}</td><td className="right money" style={{ color: l.rejectedQty ? 'var(--danger)' : 'var(--ink-5)' }}>{l.rejectedQty ? fmtQty(l.rejectedQty) : '—'}</td><td className="right money">{l.heldQty ? fmtQty(l.heldQty) : '—'}</td><td>{l.disposition ?? '—'}</td><td style={{ fontSize: 12 }}>{whName(l.warehouseId)}{l.bin ? ` · ${l.bin}` : ''}</td><td className="identifier" style={{ fontSize: 12 }}>{l.batch ?? (l.serials?.length ? `${l.serials.length} serials` : '—')}{l.expiryDate ? ` · exp ${fmtDate(l.expiryDate)}` : ''}</td><td className="right money">{fmtMoney(l.rate, g.currency)}</td><td className="right money">{fmtMoney(l.taxable, g.currency)}</td><td className="right money">{fmtQty(l.invoicedQty ?? 0)}</td><td style={{ fontSize: 12, color: 'var(--ink-3)' }}>{l.qcNote ?? '—'}</td></tr>)}</tbody></table></div>
            </div>) },
