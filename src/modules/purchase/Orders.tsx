...
export function OrderRegister() {
  ...
  const columns: Column<PurchaseOrder>[] = [
    { key: 'number', label: 'Number', sortable: true, render: (r) => <span className="identifier link">{r.number}{r.revision ? <span style={{ color: 'var(--ink-3)', fontWeight: 400 }}> · rev {r.revision}</span> : null}</span> },
    ...
    { key: 'accepted', label: 'Accepted / rejected', align: 'right', value: (r) => A.poFulfilment(r).accepted, render: (r) => { const f = A.poFulfilment(r); return <span className="money">{f.accepted}{f.rejected ? <span style={{ color: 'var(--danger)' }}> / {f.rejected}</span> : ''}</span>; } },
    ...
    { key: 'returned', label: 'Returned', align: 'right', value: (r) => A.poFulfilment(r).returned, render: (r) => { const f = A.poFulfilment(r); return f.returned ? <span className="money" style={{ color: 'var(--warn)' }}>{f.returned}</span> : <span style={{ color: 'var(--ink-5)' }}>—</span>; } },
    { key: 'pending', label: 'Pending qty', align: 'right', value: (r) => A.poFulfilment(r).pending, render: (r) => { const f = A.poFulfilment(r); return f.pending ? <span className="money">{f.pending}</span> : <span style={{ color: 'var(--ink-5)' }}>—</span>; } },
  ...
  return (
    <RegisterPage<PurchaseOrder>
      title="Purchase orders" subtitle={<>{mine.length} orders · <span style={{ fontVariantNumeric: 'tabular-nums' }}>{fmtMoney(apOutstanding)}</span> AP outstanding · {s.branch?.name} · FY {s.state.fy}</>} entity="purchase orders" rows={mine} columns={columns} searchKeys={['number', 'partyName', 'reference', 'requisitionNumber']}
