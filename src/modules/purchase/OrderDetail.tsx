...
export function OrderDetail({ id }: { id: string }) {
  ...
  const footer = (
    <>
      <div style={{ flex: 1, fontSize: 12, color: 'var(--ink-3)' }}>{pending ? `Awaiting ${pending.steps.find((x) => x.order === pending.currentStep)?.approverLabel}` : po.status === 'Approved' ? 'Approved — ready for receipt' : ''}</div>
      ...
      {pending && <Button variant="tinted" tone="danger" disabled={!canAct.ok} reason={canAct.reason} onClick={() => confirm.open({ title: `Reject ${po.number}?`, reasonRequired: true, confirmLabel: 'Reject PO', danger: true, onConfirm: (r) => engine.actOnApproval(pending.id, 'Reject', { comment: r }) })}>Reject</Button>}
        ...
        rail={<StandardRail doc={po} facts={[{ k: 'Expected', v: fmtDate(po.expectedDate) }, { k: 'Terms', v: po.paymentTerms ?? '—' }, ...(po.requisitionNumber ? [{ k: 'Requisition', v: <DocLink path={`purchase/requisitions/${po.requisitionId}`} number={po.requisitionNumber} /> }] : []), ...(po.rfqNumber ? [{ k: 'RFQ', v: <DocLink path={`purchase/rfqs/${po.rfqId}`} number={po.rfqNumber} /> }] : []), ...(po.dimensions?.Department ? [{ k: 'Department', v: db.find<any>(C.dimensions, po.dimensions.Department)?.name }] : []), ...(po.dimensions?.Project ? [{ k: 'Project', v: db.find<any>(C.dimensions, po.dimensions.Project)?.name }] : [])]}>
          <RailSection label="Fulfilment">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12 }}>
              ...
              {f.rejected > 0 && <div style={{ color: 'var(--danger)' }}>Rejected at QC: {f.rejected}</div>}
              {f.returned > 0 && <div style={{ color: 'var(--warn)' }}>Returned: {f.returned}</div>}
            </div>
          </RailSection>
        ...
        tabs={[
          { id: 'lines', label: 'Lines', content: (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <LineItemGrid lines={po.lines} readOnly direction="purchase" currency={po.currency} showWarehouse showDiscount showTax totals={po.totals} extraColumns={[{ key: 'expected', label: 'Expected', width: 100, render: (l) => <span style={{ fontSize: 12 }}>{fmtDate((l as any).expectedDate ?? po.expectedDate)}</span> }, { key: 'recv', label: 'Recv / Acc / Rej', width: 130, render: (l) => <span className="money" style={{ fontSize: 12 }}>{fmtQty(l.receivedQty ?? 0)} / <span style={{ color: 'var(--good)' }}>{fmtQty(l.acceptedQty ?? 0)}</span> / <span style={{ color: (l.rejectedQty ?? 0) > 0 ? 'var(--danger)' : 'var(--ink-5)' }}>{fmtQty(l.rejectedQty ?? 0)}</span></span> }, { key: 'inv', label: 'Invoiced', width: 80, render: (l) => <span className="money" style={{ fontSize: 12 }}>{fmtQty(l.invoicedQty ?? 0)}</span> }, { key: 'pend', label: 'Pending', width: 80, render: (l) => { const p = Math.max(0, l.qty - (l.acceptedQty ?? 0) - ((l as any).cancelledQty ?? 0)); return <span className="money" style={{ fontSize: 12, color: p > 0 ? 'var(--warn)' : 'var(--ink-5)' }}>{fmtQty(p)}</span>; } }]} />
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 16 }}>
                <div className="card" style={{ padding: 16 }}><div className="section-title">Tax breakup</div><TaxBreakup totals={po.totals} currency={po.currency} />{po.terms && <div style={{ marginTop: 12, fontSize: 12, color: 'var(--ink-3)' }}><strong>Terms:</strong> {po.terms}</div>}{po.notes && <div style={{ marginTop: 6, fontSize: 12, color: 'var(--ink-3)' }}><strong>Notes:</strong> {po.notes}</div>}</div>
          ...
          { id: 'related', label: `Receipts & invoices (${grns.length + invoices.length})`, content: (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div className="card" style={{ overflow: 'hidden' }}><div style={{ padding: '10px 16px', borderBottom: '1px solid var(--line)', fontWeight: 600, fontSize: 13 }}>Goods receipts</div>
                {grns.length === 0 ? <div style={{ padding: 16, fontSize: 13, color: 'var(--ink-3)' }}>No receipts yet.{canReceive && <> <button type="button" className="btn-link" onClick={() => nav.go(`purchase/grn/new?po=${id}`)}>Receive goods</button></>}</div> : <table className="data-table dense"><thead><tr><th>GRN</th><th>Date</th><th className="right">Received</th><th className="right">Accepted</th><th className="right">Rejected</th><th>QC</th><th>Status</th></tr></thead><tbody>{grns.map((g) => <tr key={g.id} className="clickable" onClick={() => nav.go(`purchase/grn/${g.id}`)}><td className="identifier link">{g.number}</td><td>{fmtDate(g.date)}</td><td className="right money">{g.lines.reduce((x, l) => x + l.receivedQty, 0)}</td><td className="right money">{g.lines.reduce((x, l) => x + l.acceptedQty, 0)}</td><td className="right money">{g.lines.reduce((x, l) => x + l.rejectedQty, 0)}</td><td><Badge status={g.qcStatus} /></td><td><Badge status={g.status} /></td></tr>)}</tbody></table>}</div>
              <div className="card" style={{ overflow: 'hidden' }}><div style={{ padding: '10px 16px', borderBottom: '1px solid var(--line)', fontWeight: 600, fontSize: 13 }}>Vendor invoices</div>
                {invoices.length === 0 ? <div style={{ padding: 16, fontSize: 13, color: 'var(--ink-3)' }}>No invoices booked.{canInvoice && <> <button type="button" className="btn-link" onClick={() => nav.go(`purchase/vendor-invoices/new?po=${id}`)}>Book vendor invoice</button></>}</div> : <table className="data-table dense"><thead><tr><th>Invoice</th><th>Supplier ref</th><th>Date</th><th className="right">Total</th><th>Match</th><th>Status</th></tr></thead><tbody>{invoices.map((v) => <tr key={v.id} className="clickable" onClick={() => nav.go(`purchase/vendor-invoices/${v.id}`)}><td className="identifier link">{v.number}</td><td className="identifier">{v.supplierInvoiceNumber}</td><td>{fmtDate(v.date)}</td><td className="right money">{fmtMoney(v.totals.total, v.currency)}</td><td><Badge status={v.matchStatus} /></td><td><Badge status={v.status} /></td></tr>)}</tbody></table>}</div>
              {po.amendments.length > 0 && <div className="card" style={{ padding: 16 }}><div className="section-title">Amendments</div>{po.amendments.map((a) => <div key={a.revision} style={{ fontSize: 12, marginBottom: 8 }}><strong>Rev {a.revision}</strong> · {fmtDate(a.at)} · {a.by} — {a.summary}<div style={{ color: 'var(--ink-3)' }}>{a.reason}</div></div>)}</div>}
            </div>) },
