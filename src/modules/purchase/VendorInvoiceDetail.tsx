  ...
  return (
    <>
      <DocumentPage backLabel="Vendor invoices" onBack={() => nav.go('purchase/vendor-invoices')} number={v.number === 'VINV/DRAFT' ? `Draft · ${v.supplierInvoiceNumber}` : v.number} activeTab={tab} onTab={setTab}
        ...
        tabs={[
          { id: 'lines', label: 'Lines', content: (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <LineItemGrid lines={v.lines} readOnly direction="purchase" currency={v.currency} showWarehouse={!v.poId} showDiscount showTax showAccount totals={v.totals} extraColumns={v.poId ? [{ key: 'po', label: 'PO rate · qty', width: 120, render: (l) => { const x = l as VendorInvoiceLine; return <span style={{ fontSize: 12, color: x.poRate !== undefined && x.rate !== x.poRate ? '#C0393F' : '#5F6368' }}>{fmtMoney(x.poRate ?? 0, v.currency)} · {fmtQty(x.poQty ?? 0)}</span>; } }, { key: 'grn', label: 'GRN qty', width: 80, render: (l) => <span style={{ fontSize: 12, color: '#5F6368' }}>{(l as VendorInvoiceLine).grnQty ?? '—'}</span> }] : undefined} />
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 16 }}>
                <div className="card" style={{ padding: 16 }}><div className="section-title">Input tax breakup</div><TaxBreakup totals={v.totals} currency={v.currency} />{(v.charges ?? []).length > 0 && <div style={{ marginTop: 12, fontSize: 12 }}><strong>Charges:</strong> {(v.charges ?? []).map((c) => `${c.name} ${fmtMoney(c.amount, v.currency)}`).join(' · ')}</div>}{v.notes && <div style={{ marginTop: 6, fontSize: 12, color: '#5F6368' }}>{v.notes}</div>}</div>
        ...
        footer={<>
          <div style={{ flex: 1, fontSize: 12, color: '#5F6368' }}>{v.status === 'Posted' ? `Posted ${fmtDate(v.postedAt)} · ${v.journalNumber}` : block ?? ''}</div>
