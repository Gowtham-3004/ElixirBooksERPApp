...
function RequisitionDetail({ id }: { id: string }) {
  ...
  const footer = (
    <>
      ...
      {doc.status === 'Submitted' && <Button variant="tinted" tone="danger" disabled={!canApprove} reason={!canApprove ? 'Requires approver role' : undefined} onClick={() => confirm.open({ title: `Reject ${doc.number}?`, reasonRequired: true, confirmLabel: 'Reject requisition', danger: true, onConfirm: (r) => A.decideRequisition(id, 'Rejected', r) })}>Reject</Button>}
      {doc.status === 'Submitted' && <Button variant="primary" tone="good" disabled={!canApprove} reason={!canApprove ? 'Requires approver role' : undefined} onClick={() => confirm.open({ title: `Approve ${doc.number}?`, statement: `${doc.requesterName} · ${fmtMoney(doc.totals.total)} · need by ${fmtDate(doc.needByDate)}`, consequences: [{ engine: 'Workflow', text: 'Requisition becomes Approved and can be converted to a PO or RFQ' }], confirmLabel: 'Approve requisition', onConfirm: (r) => A.decideRequisition(id, 'Approved', r) })}>Approve</Button>}
