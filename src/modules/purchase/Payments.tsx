...
export function PaymentRegister() {
  ...
  return (
    <RegisterPage<Payment> title="Supplier payments" subtitle={<>{mine.length} payments · <span style={{ fontFeatureSettings: '"tnum" 1' }}>{fmtMoney(paid)}</span> paid · {s.company?.tradeName} · FY {s.state.fy}</>} entity="payments" rows={mine} searchKeys={['number', 'partyName', 'utr', 'batchNumber']}
      columns={[
        ...
        { key: 'utr', label: 'UTR / Ref', render: (r) => <span className="identifier" style={{ fontSize: 11, color: '#6E6E71' }}>{r.utr ?? '—'}</span> },
        ...
        { key: 'tds', label: 'TDS', align: 'right', value: (r) => r.tdsAmount, render: (r) => r.tdsAmount ? <span className="money" style={{ color: '#C0393F' }}>{fmtMoney(r.tdsAmount, r.currency)}</span> : <span style={{ color: '#B0B5BF' }}>—</span> },
  ...
  return (
    <div className="page">
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ padding: '10px 16px', borderBottom: '1px solid #EAEAEA', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontWeight: 600, fontSize: 13 }}>Open items · {p.partyName ?? 'choose a supplier'}</span><div style={{ display: 'flex', gap: 8 }}><Button size="sm" onClick={() => set({ allocations: open.filter((o) => o.direction === 'Debit').map((o) => A.allocationFor(o, o.outstanding)) })} disabled={!open.length}>Allocate all invoices</Button><Button size="sm" variant="ghost" onClick={() => set({ allocations: [] })}>Clear</Button></div></div>
        {!p.partyId ? <div style={{ padding: 16, fontSize: 13, color: '#5F6368' }}>Choose a supplier to see payable invoices, debit notes and advances.</div> : open.length === 0 ? <div style={{ padding: 16, fontSize: 13, color: '#5F6368' }}>No open items — enter an advance amount to pay on account.</div> : (
          <table className="data-table dense">
            ...
            <tbody>{open.map((o) => { const a = alloc(o); const od = o.direction === 'Debit' ? daysBetween(o.dueDate, today()) : 0; return (
              <tr key={o.id} className={a ? 'selected' : ''}>
                ...
                <td>{fmtDate(o.date)}</td><td>{o.direction === 'Debit' ? <span style={{ fontSize: 12, color: od > 0 ? '#C0393F' : undefined }}>{fmtDate(o.dueDate)}{od > 0 ? ` · ${od} d overdue` : ''}</span> : '—'}</td>
                ...
                <td className="right money" style={{ color: '#C0393F' }}>{a?.tds ? fmtMoney(a.tds, p.currency) : '—'}</td>
              </tr>); })}</tbody>
          </table>)}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 380px', gap: 16 }}>
        ...
        <div className="card" style={{ padding: 16 }}>
          ...
          {p.currency !== s.currency && <div style={{ fontSize: 12, color: '#5F6368', marginTop: 8 }}>≈ {fmtMoney(p.netAmount * p.rate, s.currency)} @ {p.rate}</div>}
        </div>
      </div>
