...
export function VendorInvoiceRegister() {
  ...
  const columns: Column<VendorInvoice>[] = [
    ...
    { key: 'grn', label: 'Source GRN', render: (r) => r.grnNumbers.length ? <DocLink path={`purchase/grn/${r.grnIds[0]}`} number={r.grnNumbers.join(', ')} /> : <span style={{ color: '#B0B5BF' }}>—</span> },
    { key: 'taxable', label: 'Taxable', align: 'right', value: (r) => r.totals.taxable, render: (r) => <Money value={r.totals.taxable} currency={r.currency} /> },
    { key: 'tax', label: 'Tax', align: 'right', value: (r) => r.totals.tax, render: (r) => <span className="money" style={{ color: '#5F6368' }}>{fmtMoney(r.totals.tax, r.currency)}{r.reverseCharge ? ' (RCM)' : ''}</span> },
    { key: 'tds', label: 'TDS', align: 'right', value: (r) => r.totals.tds, render: (r) => r.totals.tds ? <span className="money" style={{ color: '#C0393F' }}>{fmtMoney(r.totals.tds, r.currency)}</span> : <span style={{ color: '#B0B5BF' }}>—</span> },
  ...
  return (
    <RegisterPage<VendorInvoice> title="Vendor invoices" subtitle={<>{mine.length} invoices · <span style={{ fontFeatureSettings: '"tnum" 1' }}>{fmtMoney(payable)}</span> outstanding · {s.company?.tradeName} · FY {s.state.fy}</>} entity="vendor invoices" rows={mine} columns={columns} searchKeys={['number', 'supplierInvoiceNumber', 'partyName', 'poNumber']}
