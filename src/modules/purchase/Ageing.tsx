...
const BUCKETS: { key: 'current' | 'd030' | 'd3160' | 'd6190' | 'd90p'; label: string; color: string }[] = [
  { key: 'current', label: 'Current', color: '#12784E' }, { key: 'd030', label: '1–30 days', color: '#F97316' }, { key: 'd3160', label: '31–60 days', color: '#E07014' }, { key: 'd6190', label: '61–90 days', color: '#C0393F' }, { key: 'd90p', label: '>90 days', color: '#9B1B21' },
];
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, minmax(0,1fr))', gap: 12 }}>
        {BUCKETS.map((b) => <div key={b.key} className="kpi-tile"><div className="section-label">{b.label}</div><div style={{ fontSize: 16, fontWeight: 600, color: b.color, fontFeatureSettings: '"tnum" 1' }}>{fmtMoney(tot(b.key))}</div></div>)}
        <div className="kpi-tile"><div className="section-label">Total (net of credits)</div><div style={{ fontSize: 16, fontWeight: 600, fontFeatureSettings: '"tnum" 1' }}>{fmtMoney(tot('total'))}</div><div style={{ fontSize: 11, color: '#5F6368' }}>credits {fmtMoney(tot('credits'))}</div></div>
      </div>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.supplierId} className="clickable" onClick={() => nav.go(`purchase/ageing/${r.supplierId}`)}>
                  ...
                  {BUCKETS.map((b) => <td key={b.key} className="right"><span className="money" style={{ color: r[b.key] > 0 ? b.color : '#B0B5BF' }}>{r[b.key] > 0 ? fmtMoney(r[b.key]) : '—'}</span></td>)}
                  <td className="right"><span className="money" style={{ color: r.credits ? '#12784E' : '#B0B5BF' }}>{r.credits ? '−' + fmtMoney(r.credits) : '—'}</span></td>
                  <td className="right"><span className="money" style={{ fontWeight: 600 }}>{fmtMoney(r.total)}</span></td>
                  <td className="right">{r.overdue > 0 ? <span className="money" style={{ color: '#C0393F', fontWeight: 600 }}>{fmtMoney(r.overdue)}</span> : <span style={{ color: '#12784E', fontSize: 13 }}>✓ Current</span>}</td>
              ...
              <tr style={{ background: '#F9FBFC', fontWeight: 600 }}><td>Total</td>{BUCKETS.map((b) => <td key={b.key} className="right money">{fmtMoney(tot(b.key))}</td>)}<td className="right money">−{fmtMoney(tot('credits'))}</td><td className="right money">{fmtMoney(tot('total'))}</td><td className="right money" style={{ color: '#C0393F' }}>{fmtMoney(tot('overdue'))}</td><td /></tr>
            </tbody>
          </table>
        </div>)}
      <Modal open={proposal} onClose={() => setProposal(false)} title="Create payment proposal" description="Select the invoices to pay. Lines are grouped by supplier into a batch that goes through maker-checker approval." width={860}
        ...
        <div style={{ marginTop: 10, fontSize: 12, color: '#5F6368' }}>{selected.size} invoice(s) · {fmtMoney(selectedItems.reduce((x, o) => x + o.outstanding, 0))} · {new Set(selectedItems.map((o) => o.partyId)).size} supplier(s)</div>
      </Modal>
    </div>
  );
}
...
function SupplierStatement({ supplierId, asAt }: { supplierId: string; asAt: string }) {
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div><button type="button" className="btn-link" style={{ color: '#5F6368' }} onClick={() => nav.go('purchase/ageing')}>← AP ageing</button><h1 className="page-title">Statement · {sup.name}</h1><div className="page-subtitle"><ScopeLine extra={`GSTIN ${sup.gstin ?? '—'} · ${sup.purchaseTerms} · as at ${fmtDate(asAt)}`} /></div></div>
