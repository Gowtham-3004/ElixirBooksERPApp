  ...
  return (
    <RegisterPage<DebitNote> title="Debit notes & purchase returns" subtitle={`${mine.length} debit notes · ${returns.length} goods returns · ${s.company?.tradeName}`} entity="debit notes" rows={mine} searchKeys={['number', 'partyName', 'invoiceNumber', 'reasonCode']}
      columns={[
        ...
        { key: 'goodsReturn', label: 'Goods', render: (r) => r.goodsReturn ? <Badge status="Returned">Returned</Badge> : <span style={{ color: '#5F6368', fontSize: 12 }}>No</span> },
  ...
  return (
    <div className="page">
      ...
      <Modal open={pick} onClose={() => setPick(false)} title="Debit note against" description="Pick the posted vendor invoice (value / price corrections) or GRN (rejected / returned goods)." width={720} footer={<Button onClick={() => setPick(false)}>Continue without source</Button>}>
        ...
        <div className="card" style={{ overflow: 'auto', maxHeight: 220, marginBottom: 14 }}><table className="data-table dense"><tbody>{invoices.map((v) => <tr key={v.id} className="clickable" onClick={() => { setD(A.computeDebitNote(A.newDebitNote({ invoiceId: v.id }))); setPick(false); }}><td className="identifier">{v.number}</td><td>{v.partyName}</td><td>{fmtDate(v.date)}</td><td className="right money">{fmtMoney(v.totals.total, v.currency)}</td><td className="right money" style={{ color: '#5F6368' }}>due {fmtMoney(v.totals.due, v.currency)}</td></tr>)}</tbody></table></div>
