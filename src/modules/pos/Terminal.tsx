  ...
  return (
    <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--surface-2)', padding: 16, overflow: 'auto' }}>
      <div style={{ width: 420, maxWidth: '100%', display: 'flex', flexDirection: 'column', gap: 16, alignItems: 'center' }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'var(--accent)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><MonitorIcon size={28} /></div>
        ...
        <div className="card" style={{ padding: 24, width: '100%', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}><span style={{ color: 'var(--ink-3)' }}>Cashier</span><span style={{ fontWeight: 500 }}>{s.user?.name}</span></div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}><span style={{ color: 'var(--ink-3)' }}>Branch · warehouse</span><span style={{ fontWeight: 500 }}>{db.find<any>(C.branches, terminals.find((t) => t.id === terminalId)?.branchId)?.name ?? '—'} · {db.find<any>(C.warehouses, terminals.find((t) => t.id === terminalId)?.warehouseId)?.name ?? '—'}</span></div>
...
function Register({ shift }: { shift: PosShift }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      <div className="pos-topbar" style={{ height: 48, background: 'var(--ink)', color: '#fff', display: 'flex', alignItems: 'center', gap: 16, padding: '0 16px', fontSize: 13, flexShrink: 0 }}>
      ...
      <div className="pos-register" style={{ flex: 1, display: 'flex', minHeight: 0 }}>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 10 }}>
              {visible.map((it) => { const pos = it.isStock ? engine.stockPosition(it.id, terminal?.warehouseId ?? cfg.posDefaultWarehouseId ?? '') : undefined; return (
                <button key={it.id} type="button" onClick={() => add(it)} className="card" style={{ padding: 12, textAlign: 'left', cursor: 'pointer', minHeight: 96, display: 'flex', flexDirection: 'column', gap: 4, fontFamily: 'inherit', border: '1px solid var(--line)' }} data-testid={`pos-tile-${it.code}`}>
                  <div style={{ fontSize: 10, color: 'var(--ink-3)', textTransform: 'uppercase', letterSpacing: '.04em' }}>{it.group ?? 'Other'}</div>
                  <div style={{ fontSize: 13, fontWeight: 600, lineHeight: 1.3 }}>{it.name}</div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginTop: 'auto' }}><span className="money" style={{ fontSize: 15, fontWeight: 700 }}>{fmtMoney(it.retailPrice)}</span><span style={{ fontSize: 10, color: pos && pos.onHand <= 0 ? 'var(--danger)' : 'var(--ink-4)' }}>{pos ? `${fmtQty(pos.onHand)} ${it.baseUom}` : it.baseUom}</span></div>
                </button>); })}
              {visible.length === 0 && <div style={{ gridColumn: '1 / -1', color: 'var(--ink-3)', textAlign: 'center', padding: 32 }}>No items match</div>}
            </div>
          </div>
        </div>
        {/* cart 35% */}
        <div className="pos-cart" style={{ flex: '0 0 35%', display: 'flex', flexDirection: 'column', minWidth: 0, background: '#FFF', borderRight: '1px solid var(--line)' }}>
          ...
          <div className="pos-cart-lines" style={{ flex: 1, overflow: 'auto' }}>
            {computed.length === 0 && <div style={{ padding: 40, textAlign: 'center', color: 'var(--ink-5)' }}>Scan or tap items to add them</div>}
            {computed.map((l) => (
              <div key={l.id} className="pos-line" style={{ display: 'grid', gridTemplateColumns: '1fr 112px 64px 90px 28px', gap: 8, alignItems: 'center', padding: '8px 12px', borderBottom: '1px solid var(--hairline)', fontSize: 13 }} data-testid="pos-cart-line">
                <div style={{ minWidth: 0 }}><div style={{ fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.itemName}</div><div style={{ fontSize: 11, color: 'var(--ink-3)' }}>{fmtMoney(l.rate)} × {fmtQty(l.qty)} · GST {l.taxRate}% incl.</div></div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><button type="button" style={qtyBtn} onClick={() => setQty(l.id, l.qty - 1)}>−</button><input value={l.qty} onChange={(e) => setQty(l.id, Math.max(0, Number(e.target.value) || 0))} style={{ width: 40, height: 32, textAlign: 'center', border: '1px solid var(--line)', borderRadius: 6, fontFamily: 'inherit', fontVariantNumeric: 'tabular-nums' }} /><button type="button" style={qtyBtn} onClick={() => setQty(l.id, l.qty + 1)}>+</button></div>
                ...
                <button type="button" onClick={() => setQty(l.id, 0)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink-5)', display: 'flex' }}><XIcon size={14} /></button>
              </div>
            ))}
          </div>
          <div style={{ borderTop: '1px solid var(--line)', padding: '10px 12px', fontSize: 13, display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <div className="pos-tender" style={{ flex: '1 1 25%', display: 'flex', flexDirection: 'column', background: '#FFF', padding: 14, gap: 12, minWidth: 0 }}>
          ...
          {tender === 'Card' && <><div style={{ background: 'linear-gradient(135deg, #1A3BCC, var(--accent))', color: '#fff', borderRadius: 12, padding: 14, fontSize: 12 }}><div style={{ opacity: .7 }}>CARD PAYMENT</div><div style={{ fontSize: 18, letterSpacing: 2, marginTop: 6 }}>•••• •••• •••• {cardLast4 || '____'}</div><div style={{ marginTop: 6 }}>{fmtMoney(totals.total)}</div></div><TextField label="Last 4 digits" value={cardLast4} onChange={(v) => setCardLast4(v.replace(/\D/g, '').slice(0, 4))} placeholder="1234" /><TextField label="Approval / RRN" value={ref} onChange={setRef} placeholder="Optional" /></>}
          ...
          {tender === 'Mixed' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <Row k="Tendered" v={<span style={{ color: Math.abs(tenderSum - totals.total) > 0.005 && tenderSum < totals.total ? 'var(--danger)' : 'var(--good)' }}>{fmtMoney(tenderSum)}</span>} />
              {tenderSum > totals.total && <Row k="Change (cash)" v={fmtMoney(change)} />}
            </div>
          )}
          <div style={{ marginTop: 'auto' }}>
            {errors.length > 0 && lines.length > 0 && <div style={{ fontSize: 12, color: 'var(--danger)', marginBottom: 6 }}>{errors[0]}</div>}
      ...
      <Drawer open={showHeld} onClose={() => setShowHeld(false)} title={`Held carts (${held.length})`} width={480}>
        {held.length === 0 && <div style={{ color: 'var(--ink-3)' }}>No held carts on this shift.</div>}
        {held.map((h) => <div key={h.id} className="card" style={{ padding: 12, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }}><div style={{ flex: 1 }}><div style={{ fontWeight: 600 }}>{h.label}</div><div style={{ fontSize: 12, color: 'var(--ink-3)' }}>{h.lines.length} item(s) · {fmtMoney(h.total)} · {fmtDateTime(h.heldAt)}{h.customerName ? ` · ${h.customerName}` : ''}</div></div><Button size="sm" variant="primary" onClick={() => resume(h)} data-testid="pos-resume">Resume</Button><Button size="sm" variant="ghost" onClick={() => db.remove(C.posHeldCarts, h.id)}>Discard</Button></div>)}
      </Drawer>
...
function ReceiptScreen({ bill, onNew, onPrint, print, onClosePrint }: { bill: PosBill; onNew: () => void; onPrint: () => void; print: boolean; onClosePrint: () => void }) {
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, background: 'var(--good-bg)' }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--ink-4)' }}>Journal {bill.journalNumber} · {bill.lines.length} line(s) · {bill.partyName}</div>
      ...
      <Modal open={print} onClose={onClosePrint} title={`Receipt ${bill.number}`} width={620} footer={<><Button variant="secondary" onClick={onClosePrint}>Close</Button><Button variant="primary" onClick={() => { engine.audit({ action: 'pos.bill_printed', objectType: 'POS Bill', objectId: bill.id, objectNumber: bill.number }); window.print(); }}>Print</Button></>}>
        <div style={{ background: 'var(--surface-3)', padding: 12, maxHeight: '60vh', overflow: 'auto' }}><div style={{ transform: 'scale(.72)', transformOrigin: 'top left', width: 794 }}><PrintSheet doc={bill} title="Retail invoice" partyLabel="Customer" extraHeader={<div>{bill.cashierName} · {bill.tenders.map((t) => t.type).join('+')}</div>} /></div></div>
      </Modal>
    </div>
  );
}

function Row({ k, v }: { k: string; v: React.ReactNode }) { return <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ color: 'var(--ink-3)' }}>{k}</span><span className="money">{v}</span></div>; }
...
const qtyBtn: React.CSSProperties = { width: 32, height: 32, border: '1px solid var(--line)', borderRadius: 6, background: 'var(--surface-2)', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'inherit' };
