  ...
  return (
    <>
      <DocumentPage backLabel="POS shifts" onBack={() => nav.go('pos/shifts')} number={shift.number} badges={<Badge status={shift.status === 'Pending Approval' ? 'Submitted' : shift.status === 'Closed' ? 'Closed' : 'Open'}>{shift.status}</Badge>} amount={{ label: 'Net sales', value: sum.net, currency: s.currency }}
        ...
        tabs={[
          { id: 'summary', label: 'Summary', content: <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <Card padding={0} title={undefined}><table className="data-table dense"><thead><tr><th>Tender</th><th className="right">Expected</th><th className="right">Counted</th><th className="right">Variance</th></tr></thead><tbody>{tenderRows.map((r) => <tr key={r.tender}><td>{r.tender}{r.tender === 'Cash' ? <span style={{ color: 'var(--ink-4)', fontSize: 11 }}> (incl. float)</span> : null}</td><td className="right money">{fmtMoney(r.expected)}</td><td className="right money">{r.counted !== undefined ? fmtMoney(r.counted) : '—'}</td><td className="right money">{r.variance !== undefined ? <Money value={r.variance} tone="auto" /> : '—'}</td></tr>)}</tbody></table></Card>
...
export function ShiftCloseDialog({ shift, onClose, onClosed }: { shift: PosShift; onClose: () => void; onClosed: () => void }) {
  ...
  return (
    <Modal open onClose={onClose} title={`Close shift ${shift.number}?`} description={`${sum.bills.length} bills · ${fmtMoney(sum.sales)} sales · ${sum.returns.length} returns. Count each tender; variance beyond ${fmtMoney(tol)} routes to approval.`} width={640}
      ...
      <table className="data-table dense" style={{ border: '1px solid var(--hairline)', borderRadius: 8 }}>
        ...
        <tbody>{variance.map((v) => <tr key={v.t}><td>{v.t}{v.t === 'Cash' ? <span style={{ fontSize: 11, color: 'var(--ink-4)' }}> (incl. float {fmtMoney(shift.openingFloat)})</span> : null}</td><td className="right money">{fmtMoney(v.expected)}</td><td className="right"><NumberField size="grid" value={v.counted} onChange={(x) => setCounted((c) => ({ ...c, [v.t]: x }))} decimals={2} min={0} /></td><td className="right money" style={{ color: Math.abs(v.variance) > 0.005 ? (v.variance < 0 ? 'var(--danger)' : 'var(--warn)') : 'var(--good)' }}>{fmtMoney(v.variance)}</td></tr>)}</tbody>
        <tfoot><tr><td colSpan={3}>Total variance</td><td className="right money" style={{ fontWeight: 600, color: beyond ? 'var(--danger)' : undefined }}>{fmtMoney(total)}</td></tr></tfoot>
      </table>
...
function ReprintModal({ bill, onClose }: { bill: PosBill; onClose: () => void }) {
  return (
    <Modal open onClose={onClose} title={`Reprint ${bill.number}`} description={`Reprinted ${bill.reprints ?? 0} time(s) — each reprint is audited.`} width={640} footer={<><Button variant="secondary" onClick={onClose}>Close</Button><Button variant="primary" onClick={() => { db.update<PosBill>(C.posBills, bill.id, { reprints: (bill.reprints ?? 0) + 1 }); engine.audit({ action: 'pos.bill_reprinted', objectType: 'POS Bill', objectId: bill.id, objectNumber: bill.number }); window.print(); }}>Print duplicate</Button></>}>
      <div style={{ background: 'var(--surface-3)', padding: 12, maxHeight: '60vh', overflow: 'auto' }}><div style={{ transform: 'scale(.72)', transformOrigin: 'top left', width: 794 }}><PrintSheet doc={bill} title="Retail invoice (duplicate)" partyLabel="Customer" /></div></div>
    </Modal>
  );
}
  ...
  return (
    <div className="page">
      ...
      <Card title="Original bill">
        ...
        {noReceipt && (
          <div style={{ marginTop: 12 }}>
            <div style={{ fontSize: 12, color: 'var(--ink-3)', marginBottom: 6 }}>Add the items being returned at current retail price.</div>
