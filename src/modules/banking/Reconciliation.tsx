...
export function ReconciliationWorkbench({ params, id }: { params: Record<string, string>; id?: string }) {
  ...
  return (
    <div className="page" style={{ gap: 12 }}>
      ...
      {summary && (
        <div style={{ background: '#F9FBFC', border: '1px solid #EAEAEA', borderRadius: 10, padding: '12px 20px', display: 'flex', gap: 32, alignItems: 'center' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(470px, 1fr))', gap: 12, alignItems: 'start' }}>
        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{ padding: '10px 14px', borderBottom: '1px solid #EAEAEA', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontWeight: 600, fontSize: 13 }}>Statement lines ({lines.length})</span><span style={{ fontSize: 12, color: '#5F6368' }}>{lines.filter((l) => l.status === 'Matched').length} matched · {lines.filter((l) => l.status === 'Unmatched').length} open</span></div>
            ...
            <table className="data-table dense">
              ...
              <tbody>{lines.map((l) => { const sg = suggestions.get(l.id); const sel = selStm.has(l.id); const open = l.status === 'Unmatched'; return (
                <tr key={l.id} className={sel ? 'selected' : ''} style={{ cursor: open ? 'pointer' : undefined, opacity: l.status === 'Excluded' ? 0.5 : 1 }} onClick={() => { if (!open) return; const n = new Set(selStm); if (n.has(l.id)) n.delete(l.id); else n.add(l.id); setSelStm(n); }}>
                  ...
                  <td style={{ fontSize: 12, color: '#5F6368', whiteSpace: 'nowrap' }}>{fmtDate(l.date)}</td>
                  <td><div className="identifier" style={{ fontSize: 12 }}>{l.description}</div><div className="identifier" style={{ fontSize: 11, color: '#6E6E71' }}>{l.reference}</div>{sg && open && <div style={{ marginTop: 3, display: 'flex', gap: 6, alignItems: 'center' }}><Pill tone={sg.confidence >= threshold ? 'good' : 'warning'}>Suggested · {sg.confidence >= 90 ? 'High' : sg.confidence >= threshold ? 'Medium' : 'Low'} {sg.confidence}%</Pill><span style={{ fontSize: 11, color: '#5F6368' }}>{book.find((b) => b.journalId === sg.journalId)?.docNumber} · {sg.reason}</span><button type="button" className="btn-link" style={{ fontSize: 11 }} onClick={(e) => { e.stopPropagation(); acceptSuggestion(l); }}>Match</button></div>}{l.status === 'Matched' && <div style={{ fontSize: 11, color: '#12784E', marginTop: 2 }}>↔ {l.matchedDocNumber}{l.matchConfidence ? ` · ${l.matchConfidence}%` : ''}{l.matchedBy ? ` · ${l.matchedBy}` : ''}</div>}</td>
                  <td className="right" style={{ whiteSpace: 'nowrap' }}><span className="money" style={{ color: l.credit ? '#12784E' : '#C0393F' }}>{fmtMoney(l.credit || l.debit, cur)} <span style={{ fontSize: 10 }}>{l.credit ? 'Cr' : 'Dr'}</span></span></td>
              ...
              {lines.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', color: '#5F6368', height: 64 }}>No statement lines</td></tr>}</tbody>
            </table>
          </div>
        </div>
        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{ padding: '10px 14px', borderBottom: '1px solid #EAEAEA', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontWeight: 600, fontSize: 13 }}>Book entries on {acc?.code} ({book.filter((b) => !b.cleared).length} uncleared)</span><Button size="sm" variant="ghost" onClick={() => openAdjust()} disabled={!selStm.size || Math.abs(diff) < 0.005} reason={!selStm.size ? 'Select statement lines' : Math.abs(diff) < 0.005 ? 'No difference' : undefined}>+ Create adjustment for difference</Button></div>
          <div style={{ maxHeight: 520, overflow: 'auto' }}>
            <table className="data-table dense">
              ...
              <tbody>{book.filter((b) => !b.cleared || (b.date >= from && b.date <= to)).map((b) => { const sel = selBook.has(b.journalId); const suggestedFor = Array.from(suggestions.entries()).find(([, sg]) => sg.journalId === b.journalId); return (
                <tr key={b.journalId} className={sel ? 'selected' : ''} style={{ cursor: !b.cleared ? 'pointer' : undefined, opacity: b.cleared ? 0.6 : 1 }} onClick={() => { if (b.cleared) return; const n = new Set(selBook); if (n.has(b.journalId)) n.delete(b.journalId); else n.add(b.journalId); setSelBook(n); }}>
                  ...
                  <td style={{ fontSize: 12, color: '#5F6368', whiteSpace: 'nowrap' }}>{fmtDate(b.date)}</td>
                  <td><div><span className="identifier link" style={{ fontSize: 12 }} onClick={(e) => { e.stopPropagation(); nav.go(`accounting/journals/${b.journalId}`); }}>{b.docNumber}</span> <span style={{ fontSize: 11, color: '#5F6368' }}>{b.sourceType}</span></div><div style={{ fontSize: 11, color: '#6E6E71', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 240 }}>{b.partyName ? `${b.partyName} · ` : ''}{b.narration}</div>{suggestedFor && !b.cleared && <div style={{ fontSize: 11, color: '#3E5BA5' }}>← suggested for {lines.find((l) => l.id === suggestedFor[0])?.description}</div>}</td>
                  <td className="right" style={{ whiteSpace: 'nowrap' }}><span className="money" style={{ color: b.amount > 0 ? '#12784E' : '#C0393F' }}>{fmtMoney(Math.abs(b.amount), cur)} <span style={{ fontSize: 10 }}>{b.amount > 0 ? 'Cr' : 'Dr'}</span></span><div>{b.cleared ? <Badge status="Matched">Cleared</Badge> : <Badge status="Unmatched">Open</Badge>}</div></td>
                </tr>); })}
              {book.length === 0 && <tr><td colSpan={4} style={{ textAlign: 'center', color: '#5F6368', height: 64 }}>No postings on this account</td></tr>}</tbody>
            </table>
          </div>
        </div>
      </div>
      {(selStm.size > 0 || selBook.size > 0) && (
        <div style={{ position: 'sticky', bottom: 0, background: '#fff', border: '1px solid #EAEAEA', borderRadius: 10, padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 16, boxShadow: '0 -4px 16px rgba(0,0,0,0.06)' }}>
          <span style={{ fontSize: 13, color: '#5F6368' }}>{selStm.size} statement line{selStm.size === 1 ? '' : 's'} ↔ {selBook.size} book entr{selBook.size === 1 ? 'y' : 'ies'}</span>
          <span style={{ fontSize: 14, fontWeight: 500, fontFeatureSettings: '"tnum" 1' }}>{fmtMoney(stmAmt, cur)} {Math.abs(diff) < 0.005 ? '=' : '≠'} {fmtMoney(bookAmt, cur)}</span>
      ...
      <div className="print-sheet card" style={{ padding: 24 }}>
        ...
        <table className="data-table dense" style={{ marginBottom: 16 }}>
          <tbody>
            ...
            <tr><td colSpan={2} style={{ background: '#F9FBFC', fontWeight: 600 }}>Less: payments / entries in books not yet in bank statement ({rec.unmatchedBook.filter((b) => b.amount < 0).length})</td></tr>
            ...
            <tr><td colSpan={2} style={{ background: '#F9FBFC', fontWeight: 600 }}>Add: receipts in books not yet in bank statement ({rec.unmatchedBook.filter((b) => b.amount > 0).length})</td></tr>
            {rec.unmatchedBook.filter((b) => b.amount > 0).map((b) => <tr key={b.journalId}><td style={{ paddingLeft: 24 }}><DocLink path={`accounting/journals/${b.journalId}`} number={b.number} /> · {fmtDate(b.date)} · {b.narration}</td><td className="right money">−{fmtMoney(b.amount)}</td></tr>)}
            <tr><td colSpan={2} style={{ background: '#F9FBFC', fontWeight: 600 }}>Add / less: bank entries not yet in books ({rec.unmatchedStatement.length})</td></tr>
        ...
        <div style={{ fontSize: 12, color: '#5F6368' }}>Cleared in period: {rec.clearedCount} line(s) · {fmtMoney(rec.clearedAmount)} · {rec.notes ?? ''}</div>
