...
function BrsReport({ rec, onBack }: { rec: Reconciliation; onBack: () => void }) {
  ...
  return (
    <div className="page">
      <div className="page-header no-print"><div><button type="button" className="btn-link" style={{ color: 'var(--ink-3)' }} onClick={onBack}>← Workbench</button><h1 className="page-title">Bank reconciliation statement · {rec.number}</h1><div className="page-subtitle">{rec.bankAccountName} · {fmtDate(rec.periodFrom)} → {fmtDate(rec.periodTo)} · {rec.status}{rec.confirmedAt ? ` · confirmed ${fmtDateTime(rec.confirmedAt)} by ${rec.confirmedBy}` : ''}</div></div><div style={{ display: 'flex', gap: 8 }}><Button onClick={() => window.print()}>Print</Button>{rec.status === 'Confirmed' && <Button variant="tinted" tone="danger" disabled={!s.can('banking.*') && !s.can('banking.reconciliation.reopen')} onClick={() => confirm.open({ title: `Reopen ${rec.number}?`, reasonRequired: true, confirmLabel: 'Reopen reconciliation', danger: true, onConfirm: (r) => { A.reopenReconciliation(rec.id, r); toast.success('Reopened'); onBack(); } })}>Reopen</Button>}</div></div>
