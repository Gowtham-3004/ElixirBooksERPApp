  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 12 }}>
        {rows.map((r) => (
          <div key={r.a.id} className="card" style={{ padding: '14px 16px', cursor: 'pointer', border: `1px solid ${sel?.a.id === r.a.id ? '#325CFF' : '#EAEAEA'}`, background: sel?.a.id === r.a.id ? '#F2F7FF' : '#fff' }} onClick={() => setActive(r.a.id)}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4, gap: 8 }}><span style={{ fontSize: 13, fontWeight: 600 }}>{r.a.bankDetails?.bankName ? `${r.a.bankDetails.bankName} ${r.a.controlType === 'Bank' ? 'Current Account' : ''}` : r.a.name}</span><Badge status={r.unmatched ? 'Pending' : 'Reconciled'}>{r.unmatched ? `${r.unmatched} unmatched` : 'Reconciled'}</Badge></div>
            <div className="identifier" style={{ fontSize: 12, color: '#6E6E71', marginBottom: 8 }}>{r.a.bankDetails ? `••••${r.a.bankDetails.accountNumber.slice(-4)} · ${r.a.bankDetails.branch}` : `${r.a.code} · ${r.a.controlType}`}{r.cur !== s.currency && <span className="currency-tag" style={{ marginLeft: 6 }}>{r.cur}</span>}</div>
            <div style={{ fontSize: 18, fontWeight: 600, fontFeatureSettings: '"tnum" 1' }}>{fmtMoney(r.bal, r.cur)}</div>
            {r.cur !== s.currency && <div style={{ fontSize: 11, color: '#5F6368' }}>≈ {fmtMoney(r.baseEq)} @ {r.fxRate}</div>}
            <div style={{ fontSize: 11, color: '#6E6E71', marginTop: 6 }}>Last reconciled {r.rec ? `${fmtDate(r.rec.periodTo)} · ${r.rec.number}` : 'never'}</div>
          </div>))}
      ...
      {sel && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16 }}>
          <div className="card" style={{ overflow: 'hidden' }}>
            <div style={{ padding: '10px 16px', borderBottom: '1px solid #EAEAEA', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontWeight: 600, fontSize: 13 }}>{sel.a.code} · {sel.a.name} — recent postings</span><div style={{ display: 'flex', gap: 6 }}><Button size="sm" onClick={() => nav.go(`banking/reconciliation?account=${sel.a.id}`)}>Reconcile</Button><Button size="sm" onClick={() => nav.go(`accounting/ledger?account=${sel.a.id}`)}>Full ledger</Button><Button size="sm" onClick={() => openEdit(sel.a)}>Edit</Button></div></div>
            <DataTable rows={ledger} rowKey={(r) => r.j.id} dense columns={[{ key: 'date', label: 'Date', render: (r) => fmtDate(r.j.date) }, { key: 'number', label: 'Journal', render: (r) => <span className="identifier link" onClick={() => nav.go(`accounting/journals/${r.j.id}`)}>{r.j.number}</span> }, { key: 'src', label: 'Source', render: (r) => <span style={{ fontSize: 12 }}>{r.j.sourceType} {r.j.sourceNumber ?? ''}</span> }, { key: 'narration', label: 'Narration', render: (r) => <span style={{ fontSize: 12, color: '#5F6368' }}>{r.j.narration}</span> }, { key: 'dr', label: 'Money in', align: 'right', render: (r) => r.dr ? <span className="money" style={{ color: '#12784E' }}>{fmtMoney(r.dr)}</span> : '—' }, { key: 'cr', label: 'Money out', align: 'right', render: (r) => r.cr ? <span className="money" style={{ color: '#C0393F' }}>{fmtMoney(r.cr)}</span> : '—' }]} emptyTitle="No postings yet" />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <KpiTile label="Book balance" value={fmtMoney(sel.bal, sel.cur)} sub={sel.cur !== s.currency ? `≈ ${fmtMoney(sel.baseEq)}` : `opening ${fmtMoney(sel.a.openingBalance ?? 0)}`} />
