  ...
  if (sel) {
    ...
    return (
      <div className="page">
        ...
        <DataTable rows={ls} dense columns={[{ key: 'date', label: 'Date', render: (l) => fmtDate(l.date) }, { key: 'description', label: 'Description', render: (l) => <span className="identifier" style={{ fontSize: 12 }}>{l.description}</span> }, { key: 'reference', label: 'Reference', render: (l) => <span className="identifier" style={{ fontSize: 11, color: 'var(--ink-4)' }}>{l.reference}</span> }, { key: 'debit', label: 'Debit', align: 'right', render: (l) => l.debit ? <span className="money" style={{ color: 'var(--danger)' }}>{fmtMoney(l.debit)}</span> : '—' }, { key: 'credit', label: 'Credit', align: 'right', render: (l) => l.credit ? <span className="money" style={{ color: 'var(--good)' }}>{fmtMoney(l.credit)}</span> : '—' }, { key: 'balance', label: 'Balance', align: 'right', render: (l) => <span className="money">{fmtMoney(l.balance ?? 0)}</span> }, { key: 'status', label: 'Match', render: (l) => <Badge status={l.status} /> }, { key: 'matchedDocNumber', label: 'Book entry', render: (l) => <span className="identifier" style={{ fontSize: 12 }}>{l.matchedDocNumber ?? '—'}</span> }]} />
      </div>
    );
  }
  return (
    <>
      <RegisterPage<BankStatement> title="Bank statements" subtitle={`${mine.length} imported · duplicate files and lines are blocked by fingerprint`} entity="statements" rows={mine} searchKeys={['fileName', 'bankAccountName']}
        columns={[
          ...
          { key: 'lineCount', label: 'Lines', align: 'right', render: (r) => { const m = lines.filter((l) => l.statementId === r.id && l.status === 'Matched').length; return <span>{r.lineCount} <span style={{ color: 'var(--ink-3)', fontSize: 12 }}>({m} matched)</span></span>; } },
    ...
    <Drawer open onClose={onClose} title="Import bank statement" subtitle={steps.map((st, i) => (i === step ? `● ${st}` : `○ ${st}`)).join('   ')} width={860}
      ...
      {step === 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <label style={{ border: '2px dashed var(--line-strong)', borderRadius: 12, padding: 40, textAlign: 'center', cursor: 'pointer', color: 'var(--ink-3)' }}><input type="file" accept=".csv,.txt" style={{ display: 'none' }} onChange={(e) => { const f = e.target.files?.[0]; if (!f) return; f.text().then((t) => load(t, f.name)); }} /><div style={{ fontSize: 28 }}>🏦</div><div style={{ fontSize: 14, color: 'var(--ink)', fontWeight: 500 }}>Drop the bank CSV here or click to choose</div><div style={{ fontSize: 12, marginTop: 4 }}>Saved formats: {formats.map((f) => f.name).join(' · ')}</div></label>
      ...
      {step === 1 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ fontSize: 13, color: 'var(--ink-3)' }}>{fileName} · {raw.length} rows · columns: {headers.join(', ')}</div>
          ...
          <div className="card" style={{ overflow: 'auto', maxHeight: 220 }}><table className="data-table dense"><thead><tr><th>Date</th><th>Description</th><th>Reference</th><th className="right">Debit</th><th className="right">Credit</th></tr></thead><tbody>{parsed.slice(0, 8).map((l) => <tr key={l.row} className={l.error ? 'error-row' : ''}><td>{l.date || <span style={{ color: 'var(--danger)' }}>?</span>}</td><td>{l.description}</td><td className="identifier">{l.reference}</td><td className="right money">{l.debit || '—'}</td><td className="right money">{l.credit || '—'}</td></tr>)}</tbody></table></div>
        </div>)}
      {(step === 2 || step === 3) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {parsed.some((l) => l.error || l.duplicate) && <div className="card" style={{ overflow: 'auto', maxHeight: 200 }}><table className="data-table dense"><thead><tr><th>Row</th><th>Issue</th></tr></thead><tbody>{parsed.filter((l) => l.error || l.duplicate).map((l) => <tr key={l.row} className={l.error ? 'error-row' : ''}><td>{l.row}</td><td style={{ color: l.error ? 'var(--danger)' : 'var(--warn)' }}>{l.error ?? l.duplicate}</td></tr>)}</tbody></table></div>}
          {step === 3 && <div style={{ fontSize: 13, color: 'var(--ink-2)' }}>Importing creates the statement <strong>{fileName}</strong> on <strong>{acc?.name}</strong> with {good.length} unmatched lines. Continue to the reconciliation workbench afterwards to match suggestions.</div>}
        </div>)}
