...
export function ActivitiesPage({ focusId }: { focusId?: string }) {
  ...
  const columns: Column<CrmActivity>[] = [
    ...
    { key: 'due', label: 'Due', sortable: true, render: (r) => { const d = r.promiseDate ?? r.dueAt; if (!d) return '—'; const late = r.status === 'Open' && d < t; return <span style={{ color: late ? 'var(--danger)' : undefined }}>{fmtDate(d)}{late ? ' · overdue' : ''}</span>; }, value: (r) => r.promiseDate ?? r.dueAt },
  ...
  return (
    <>
      <RegisterPage<CrmActivity> title="Activities" subtitle={`${rows.filter((a) => a.status === 'Open').length} open · ${overdue.length} overdue · ${mine.length} assigned to you`} rows={rows} columns={columns} entity="activities" searchKeys={['subject', 'customerName', 'leadName', 'notes']}
        headerExtra={<div className="grid-4"><KpiTile label="Overdue" value={<span style={{ color: overdue.length ? 'var(--danger)' : undefined }}>{overdue.length}</span>} sub="past due, still open" /><KpiTile label="Due today" value={dueToday.length} /><KpiTile label="Mine" value={mine.length} sub={s.user?.name} /><KpiTile label="Logged this week" value={rows.filter((a) => a.createdAt >= addDays(t, -7)).length} /></div>}
