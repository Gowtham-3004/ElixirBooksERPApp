...
export function TdsPage() {
  ...
  const cols: Column<TdsRegisterRow>[] = [
    ...
    { key: 'partyName', label: 'Deductee / party', render: (r) => <div><div className="cell-primary">{r.partyName}</div><div className="cell-secondary identifier" style={{ color: r.pan ? undefined : 'var(--danger)' }}>{r.pan ?? 'PAN missing — 20%'}</div></div>, sortable: true },
    ...
    { key: 'threshold', label: 'Threshold', align: 'right', render: (r) => <span className="money" style={{ fontSize: 12, color: r.base >= r.threshold ? 'var(--ink-3)' : 'var(--good)' }}>{r.threshold ? fmtMoney(r.threshold, s.currency) : '—'}{r.threshold && r.base < r.threshold ? ' ✓ below' : ''}</span> },
    { key: 'rate', label: 'Rate', render: (r) => (r.amount ? `${r.rate}%` : <span style={{ color: 'var(--ink-3)' }}>Nil</span>) },
    { key: 'amount', label: 'TDS / TCS', align: 'right', render: (r) => <span className="money" style={{ fontWeight: 600, color: r.amount ? 'var(--danger)' : 'var(--ink-5)' }}>{r.amount ? fmtMoney(r.amount, s.currency) : '—'}</span>, total: (rs) => <span className="money" style={{ fontWeight: 700 }}>{fmtMoney(rs.reduce((x, r) => x + r.amount, 0), s.currency)}</span> },
