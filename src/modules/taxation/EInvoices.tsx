...
export function EInvoices() {
  ...
  const cols: Column<Row>[] = [
    ...
    { key: 'irn', label: 'IRN', render: (r) => r.doc.statutory?.irn ? <span className="identifier" title={r.doc.statutory.irn} style={{ fontSize: 10, color: 'var(--ink-4)' }}>{r.doc.statutory.irn.slice(0, 24)}…</span> : <span style={{ color: 'var(--ink-5)' }}>—</span> },
    { key: 'ack', label: 'Ack no.', render: (r) => <span className="identifier" style={{ fontSize: 11 }}>{r.doc.statutory?.ackNo ?? '—'}</span> },
    { key: 'submitted', label: 'Submitted', render: (r) => <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{fmtDateTime(r.doc.statutory?.eInvoiceSubmittedAt)}</span> },
    ...
    { key: 'status', label: 'Status', render: (r) => <div><Badge status={r.status === 'Failed' ? 'Rejected' : r.status} />{(r.status === 'Rejected' || r.status === 'Failed') && r.doc.statutory?.eInvoiceError && <div style={{ fontSize: 11, color: 'var(--danger)', marginTop: 2, maxWidth: 220 }}>{r.doc.statutory.eInvoiceError}</div>}</div> },
    { key: 'act', label: '', render: (r) => (r.status === 'Pending' || r.status === 'Queued') && r.readiness.applicable ? <Button size="sm" variant="primary" disabled={!canSubmit || !r.readiness.ok} reason={!canSubmit ? 'Requires taxation permission' : !r.readiness.ok ? r.readiness.issues.join('; ') : undefined} onClick={(e) => { e.stopPropagation(); submit(r); }}>Submit</Button> : (r.status === 'Rejected' || r.status === 'Failed') ? <Button size="sm" variant="secondary" style={{ borderColor: 'var(--danger)', color: 'var(--danger)' }} onClick={(e) => { e.stopPropagation(); submit(r); }}>Fix and retry</Button> : null },
  ];
      ...
      <RegisterPage<Row> title="e-Invoices" subtitle={`IRP submission status · ${s.branch?.gstin ?? '—'} · ${period ? fmtPeriod(period) : 'all periods'}${synced ? ` · synced ${synced}` : ''}`} rows={rows} columns={cols} entity="e-invoices" searchKeys={['number', 'party']}
        ...
        headerExtra={<div style={{ display: 'flex', gap: 12, alignItems: 'flex-end' }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginLeft: 8 }}>
            {[{ label: 'Accepted', n: counts.accepted, color: 'var(--good)', bg: 'var(--good-bg)' }, { label: 'Pending', n: counts.pending, color: 'var(--info)', bg: 'var(--info-bg)' }, { label: 'Rejected', n: counts.rejected, color: 'var(--danger)', bg: 'var(--danger-bg)' }, { label: 'Cancelled', n: counts.cancelled, color: 'var(--ink-3)', bg: 'var(--surface-3)' }, { label: 'Not applicable', n: counts.na, color: 'var(--ink-3)', bg: 'var(--surface-3)' }].map((b) => <div key={b.label} style={{ padding: '6px 14px', background: b.bg, borderRadius: 8, display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 18, fontWeight: 700, color: b.color, fontVariantNumeric: 'tabular-nums' }}>{b.n}</span><span style={{ fontSize: 12, color: b.color, fontWeight: 500 }}>{b.label}</span></div>)}
          </div>
        </div>}
      ...
      <Drawer open={!!detail} onClose={() => setDetail(null)} title={detail ? `${detail.number} · e-Invoice` : ''} subtitle={detail ? `${detail.party} · ${fmtMoney(detail.total, detail.doc.currency)}` : ''} width={760}
        ...
        {detail && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><Badge status={detail.status === 'Failed' ? 'Rejected' : detail.status} />{detail.doc.statutory?.eInvoiceError && <span style={{ color: 'var(--danger)', fontSize: 13 }}>{detail.doc.statutory.eInvoiceError}</span>}</div>
            ...
            {detailLogs.length === 0 && <div style={{ fontSize: 13, color: 'var(--ink-4)' }}>No IRP calls yet for this document.</div>}
            {detailLogs.map((l) => (
              <div key={l.id} className="card" style={{ padding: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}><span style={{ fontSize: 13, fontWeight: 600 }}>{l.action} · <Badge status={l.status} /></span><span style={{ fontSize: 12, color: 'var(--ink-4)' }}>{fmtDateTime(l.at)} · {l.correlationId}</span></div>
                {l.errorMessage && <div style={{ color: 'var(--danger)', fontSize: 12, marginBottom: 6 }}>{l.errorCode} · {l.errorMessage}</div>}
                <div className="grid-2"><div><div className="section-label">Request · fingerprint {l.requestFingerprint}</div><pre style={{ fontSize: 11, background: 'var(--surface-2)', padding: 8, borderRadius: 6, overflow: 'auto', margin: 0 }}>{JSON.stringify(l.request, null, 2)}</pre></div><div><div className="section-label">Response</div><pre style={{ fontSize: 11, background: 'var(--surface-2)', padding: 8, borderRadius: 6, overflow: 'auto', margin: 0 }}>{JSON.stringify(l.response, null, 2)}</pre></div></div>
              </div>
            ))}
          </div>
        )}
    ...
    <>
      ...
      <Drawer open={!!gen} onClose={() => setGen(null)} title={`Generate e-way bill · ${gen?.number ?? ''}`} subtitle={gen ? `${gen.party} · ${fmtMoney(gen.total, gen.doc.currency)}` : ''} width={520}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {gen && <div style={{ fontSize: 12, color: 'var(--ink-4)' }}>From {db.find<any>(C.branches, gen.doc.branchId)?.name} · to {gen.row.pos} · {gen.doc.lines.length} line{gen.doc.lines.length === 1 ? '' : 's'}{gen.doc.lines.some((l) => !l.hsn) && <span style={{ color: 'var(--danger)' }}> · HSN missing on a line</span>}</div>}
        </div>
      </Drawer>
