...
export function GstRegister({ kind }: { kind: RegisterKind }) {
  ...
  const cols: Column<GstRegisterRow>[] = [
    { key: 'number', label: 'Document', render: (r) => <div><span className="identifier" style={{ color: r.sign < 0 ? 'var(--danger)' : 'var(--accent)' }}>{r.number}</span><div className="cell-secondary">{r.docType}</div></div>, sortable: true },
    ...
    { key: 'pos', label: 'POS', render: (r) => <span style={{ fontSize: 11, color: 'var(--ink-3)' }}>{r.pos}</span> },
    ...
    { key: 'cgst', label: 'CGST', align: 'right', render: (r) => <span className="money" style={{ color: 'var(--ink-3)' }}>{r.cgst ? fmtMoney(r.cgst, s.currency) : '—'}</span>, total: () => <span className="money">{fmtMoney(totals.cgst, s.currency)}</span> },
    { key: 'sgst', label: 'SGST', align: 'right', render: (r) => <span className="money" style={{ color: 'var(--ink-3)' }}>{r.sgst ? fmtMoney(r.sgst, s.currency) : '—'}</span>, total: () => <span className="money">{fmtMoney(totals.sgst, s.currency)}</span> },
    { key: 'igst', label: 'IGST', align: 'right', render: (r) => <span className="money" style={{ color: 'var(--ink-3)' }}>{r.igst ? fmtMoney(r.igst, s.currency) : '—'}</span>, total: () => <span className="money">{fmtMoney(totals.igst, s.currency)}</span> },
    { key: 'cess', label: 'Cess', align: 'right', render: (r) => <span className="money" style={{ color: 'var(--ink-3)' }}>{r.cess ? fmtMoney(r.cess, s.currency) : '—'}</span> },
