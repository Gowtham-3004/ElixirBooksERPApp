  ...
  return (
    <div className="page">
      ...
      <div className="grid-4">
        {quarters.map(([qq, l]) => <div key={qq} className="kpi-tile"><span className="section-label">{qq} · {l}</span><div style={{ fontSize: 13, marginTop: 6, display: 'flex', flexDirection: 'column', gap: 4 }}>{(['PF', 'ESI', 'PT', 'TDS'] as const).map((k) => <div key={k} style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#5F6368' }}>{k}</span><span className="money" style={{ fontWeight: 600 }}>{money(qtd(k, qq))}</span></div>)}</div></div>)}
      </div>
      <DataTable rows={rows} dense columns={[
        ...
        { key: 'due', label: 'Due date', render: (r) => <span style={{ color: !r.filing && r.due < new Date().toISOString().slice(0, 10) ? '#C0393F' : undefined }}>{fmtDate(r.due)}</span> },
