...
import { calculateRun, createRun, finalizeRun, postRun, payRun, reverseRun, bankFile, payrollEmployees, revealAudit } from './actions';
...
export function RunsPage({ id }: { id?: string }) {
  ...
  const [confirm, setConfirm] = useState<'finalize' | 'post' | 'pay' | 'reverse' | null>(null);
  ...
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              ...
              {selected.status === 'Finalized' && <><Button variant="secondary" onClick={() => setConfirm('reverse')} disabled={!canRun}>Reverse</Button><Button variant="primary" tone="good" onClick={() => setConfirm('post')} disabled={!canPost} reason={!canPost ? 'Requires posting permission' : undefined}>Post to ledger</Button></>}
              {(selected.status === 'Posted' || selected.status === 'Paid') && <><Button variant="secondary" icon={<DownloadIcon size={14} />} onClick={() => nav.go(`payroll/payslips?run=${selected.id}`)}>Payslips</Button><Button variant="secondary" onClick={() => act(() => bankFile(selected, !canView), `Bank file downloaded (${canView ? 'full' : 'masked'} account numbers)`)}>Bank file</Button>{selected.status === 'Posted' && !selected.reversalOfId && <Button variant="primary" tone="good" onClick={() => setConfirm('pay')} disabled={!canPost} reason={!canPost ? 'Requires posting permission' : undefined}>Record salary payment</Button>}{!selected.reversalOfId && !selected.reversedById && <Button variant="tinted" tone="danger" onClick={() => setConfirm('reverse')} disabled={!canPost}>Reverse run</Button>}</>}
            </div>
          </div>
      ...
      <ConfirmDialog open={confirm === 'pay'} onClose={() => setConfirm(null)} title={`Record salary payment · ${selected?.number}`} statement="Books the bank transfer for the net pay in this run and clears the per-employee liabilities raised on posting." confirmLabel="Record payment" cancelLabel="Not yet" consequences={[{ engine: 'Journal', text: `Dr salaries payable (per employee) · Cr bank — ${selected ? fmtMoney(selected.totals.net, s.currency) : ''}`, tone: 'info' }, { engine: 'Open items', text: `${selected?.lines.filter((l) => l.net > 0).length ?? 0} employee open items settled`, tone: 'success' }, { engine: 'Statutory', text: 'PF / ESI / PT / TDS payable are remitted separately under Taxation › Statutory', tone: 'info' }]} onConfirm={() => { if (!selected) return; const r = payRun(selected, { date: payDate }); toast.success(`Salaries paid · ${r.paymentJournalNumber}`, { label: 'Open journal', path: `accounting/journals/${r.paymentJournalId}` }); }}>
        <DateField label="Payment date" value={payDate} onChange={setPayDate} checkPeriod />
      </ConfirmDialog>
