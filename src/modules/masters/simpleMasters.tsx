...
export function PaymentTermsRegister() {
  const cfg: SimpleMasterConfig<PaymentTerm> = {
    ...
    columns: [
      { key: 'name', label: 'Term', sortable: true, render: (r) => <span><strong>{r.name}</strong> <span className="identifier" style={{ color: 'var(--ink-3)', marginLeft: 6 }}>{r.code}</span></span> },
...
export function SalespersonRegister() {
  const cfg: SimpleMasterConfig<Salesperson> = {
    ...
    columns: [
      { key: 'name', label: 'Salesperson', sortable: true, render: (r) => <span><strong>{r.name}</strong> <span className="identifier" style={{ color: 'var(--ink-3)', marginLeft: 6 }}>{r.code}</span></span> },
      { key: 'employee', label: 'Employee', render: (r) => db.find<any>(C.employees, r.employeeId)?.name ?? <span style={{ color: 'var(--ink-3)' }}>External</span> },
...
export function ReferencePage() {
  ...
  return (
    <div className="page">
      ...
      {tab === 'countries' && <DataTable rows={countries} dense columns={[{ key: 'code', label: 'ISO', render: (r) => <span className="identifier" style={{ fontWeight: 600 }}>{r.code}</span> }, { key: 'name', label: 'Country' }, { key: 'currency', label: 'Currency', render: (r) => <span className="currency-tag">{r.currency}</span> }, { key: 'pack', label: 'Localization pack', render: (r) => { const p = db.get<any>(C.localizationPacks).find((x) => x.country === r.code && x.status === 'Approved'); return p ? <span>{p.name} v{p.packVersion} <Badge status="Approved" /></span> : <span style={{ color: 'var(--ink-3)' }}>Generic</span>; } }, { key: 'used', label: 'Companies', render: (r) => db.get<any>(C.companies).filter((c) => c.country === r.code).map((c) => c.tradeName).join(', ') || '—' }]} />}
