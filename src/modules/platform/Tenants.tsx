...
export default function Tenants({ initialId }: { initialId?: string }) {
  ...
  const columns: Column<Tenant>[] = [
    ...
    { key: 'dates', label: 'Trial end / renewal', render: (t) => <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{t.subscriptionState === 'Trial' ? `Trial ends ${fmtDate(t.trialEndsAt)}` : t.subscriptionState === 'Grace' ? `Grace until ${fmtDate(t.graceUntil)}` : t.renewsAt ? `Renews ${fmtDate(t.renewsAt)}` : '—'}</span> },
    { key: 'usage', label: 'Usage vs limits', render: (t) => { const p = planOf(t); return <div style={{ display: 'flex', flexDirection: 'column', gap: 3, minWidth: 160 }}>{['users', 'invoicesPerMonth'].map((k) => <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11 }}><span style={{ width: 62, color: 'var(--ink-3)' }}>{k === 'users' ? 'Users' : 'Invoices'}</span><div style={{ flex: 1 }}><Meter value={t.usage[k] ?? 0} max={p?.limits[k] ?? 1} /></div><span style={{ fontVariantNumeric: 'tabular-nums' }}>{t.usage[k] ?? 0}/{p?.limits[k] ?? '—'}</span></div>)}</div>; } },
  ];
        ...
        {open && openPlan && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              {Object.entries(openPlan.limits).map(([k, max]) => <div key={k} style={{ marginBottom: 8 }}><div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}><span>{k}</span><span style={{ fontVariantNumeric: 'tabular-nums' }}>{(open.usage[k] ?? 0).toLocaleString('en-IN')} / {max.toLocaleString('en-IN')}</span></div><Meter value={open.usage[k] ?? 0} max={max} /></div>)}
            </div>
